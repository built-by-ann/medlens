...
- Any fuzzy/semantic matching of medication names (e.g. inferring a brand name and a generic name refer to the same drug), since that would risk silently hiding a real documentation inconsistency instead of surfacing it, which is exactly what this application exists to catch.
...
**`GeminiProvider`** (`app/ai/providers/gemini_provider.py`) is the default implementation, built on Google's `google-genai` SDK. Two details worth being explicit about:
...
**`OpenBioLLMProvider`** (`app/ai/providers/openbiollm_provider.py`) and **`MedGemmaProvider`** (`app/ai/providers/medgemma_provider.py`) both call a locally-running [Ollama](https://ollama.com) daemon over plain HTTP (`urllib.request`, stdlib only - no new dependency), not a hosted API. This is a deliberate, pre-scoring change from an earlier design that called both through Hugging Face's hosted Inference Providers: both hosted paths turned out to be operationally unreliable during pre-evaluation smoke testing for #89/#90 (OpenBioLLM returned `503 capacity_exhausted`; MedGemma returned "This model is busy, please try again later"), before either had scored a single benchmark case. This says nothing about either model's own extraction quality, only about a hosted-inference dependency that made evaluation itself impractical; see Limitations, below.

Both providers share one shape:

- **No credential, no lazy client.** Ollama's HTTP API needs no API key; there is nothing to check lazily, so `generate_summary` builds and sends one request directly, with no cached client object (unlike `GeminiProvider`'s `self._client`).
- **Called via `/api/chat`, not `/api/generate`.** `AIProvider.generate_summary(prompt: str) -> str` is unchanged, and `build_summary_prompt()` still returns one plain string; each provider internally wraps that string as a single user-turn message, `messages=[{"role": "user", "content": prompt}]`. `/api/chat` is what makes Ollama apply the target model's own chat template automatically - the runtime wrapping is entirely Ollama's responsibility, distinct from, and never mixed into, the benchmark prompt text itself.
- **No `format: "json"` constrained generation.** Ollama supports a JSON-constrained generation mode, but neither provider uses it: #90's benchmark needs to measure each model's actual, unassisted ability to produce the requested JSON shape, not a runtime-enforced grammar. Output cleanup is the same strictly syntactic step used before this migration: `strip_code_fences`/`extract_json_object` (`app/ai/providers/text_cleanup.py`), shared by both providers, never repairing malformed JSON or touching field content.
- **Generation parameters are hardcoded constants** (`GENERATION_PARAMS` in each provider module, sent as Ollama's `options`): `temperature=0`, `seed=0`, `num_predict=1024` - deterministic wherever Ollama honors these settings, not `Settings`/environment configuration. A future evaluation run (#89) should record these exact values alongside its results, since changing them changes what's being measured.
- **Errors are classified explicitly**, not left as one generic failure: an unreachable Ollama server, a model that isn't pulled locally (HTTP 404), a request timeout, and an empty response each raise a distinct, clear `AIProviderError` message (see Error Handling, below).

What's specific to each:

- **`OpenBioLLMProvider`** calls a local model named `openbiollm-llama3-instruct`, **not** the raw `hf.co/aaditya/OpenBioLLM-Llama3-8B-GGUF:Q4_K_M` import. That GGUF embeds no usable chat template of its own (`ollama show hf.co/aaditya/OpenBioLLM-Llama3-8B-GGUF:Q4_K_M --modelfile` reports only `TEMPLATE {{ .Prompt }}`, a bare passthrough) - verified directly: a manual test against the untemplated import asked it to extract a medication from a plain clinical sentence, and it reformulated the request as a question instead of answering it. OpenBioLLM's own model card requires the exact Meta Llama 3 Instruct template, since it's a fine-tune of Meta-Llama-3-8B-Instruct never retrained with a different conversational format. `infra/ollama/openbiollm-llama3-instruct.Modelfile` is a repo-tracked, reproducible fix: it reuses the exact same, already-pulled GGUF weights (`FROM hf.co/aaditya/OpenBioLLM-Llama3-8B-GGUF:Q4_K_M`) and attaches the correct template, copied verbatim from Ollama's own official `llama3:8b-instruct-q4_K_M` library model rather than hand-transcribed - see the Modelfile's own comments, and Manual Setup, below, for how to build it. It adds no `SYSTEM` directive of any kind.
- **`MedGemmaProvider`** calls `hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M` directly, a 4B checkpoint runnable on consumer hardware (unlike the 27B checkpoint this provider called before this migration, which no Inference Provider served at a locally-practical size either). Verified directly: `ollama show hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M --modelfile` reports a correct, embedded Gemma-style chat template (`<start_of_turn>user ... <end_of_turn>\n<start_of_turn>model`), so this GGUF is used as-is, with no custom Modelfile.
...
def build_ai_summary_service(app_settings: Settings) -> AISummaryService:
    if app_settings.ai_provider == "openbiollm":
        provider = OpenBioLLMProvider(model=app_settings.openbiollm_model, base_url=app_settings.ollama_base_url)
    elif app_settings.ai_provider == "medgemma":
        provider = MedGemmaProvider(model=app_settings.medgemma_model, base_url=app_settings.ollama_base_url)
...
**Why this abstraction exists** (Decision 15, `docs/design-decisions.md`): the project intends to evaluate more than one provider over time (see README's roadmap). Three providers are implemented today: Gemini (the default), OpenBioLLM, and MedGemma; a full benchmark comparison across all three has since been run and scored (see below). Behind this interface, adding one is a new class implementing one method and translating its own SDK's exceptions into `AIProviderError`; nothing in `AISummaryService`, the prompt template, or the API route needs to change. See Extending the AI Layer, below, for the concrete steps.

The interface pays off a second way, independently of the running application: `benchmark/runner/` (Issue #89, see `benchmark/README.md`) constructs `GeminiProvider`/`OpenBioLLMProvider`/`MedGemmaProvider` directly, not through `build_ai_summary_service()`/`get_ai_summary_service()`, so it can run several providers in one process, which `AI_PROVIDER`'s single-active-provider design isn't suited for. It reuses `build_summary_prompt()` and `ClinicalSummary` completely unmodified, and records structured predictions for later scoring. `benchmark/metrics/` (Issue #90) then grades those predictions against the benchmark's ground truth: medication-detection precision/recall/F1, attribute accuracy, reliability, latency, using the exact same `ClinicalSummary`-shaped data, and reusing `Medication`'s own field set as the basis for its matching/scoring rules; it never imports an `AIProvider` at all, since it only ever reads what the runner already produced. See `benchmark/README.md`'s "Scoring an evaluation run" section for the full mechanics, `docs/model-evaluation.md` for the full evaluation methodology narrative, and `docs/model-comparison-report.md` for the current results.
...
|---|---|---|
| Reads clinical note text and identifies medications mentioned | Yes | |
| Records what a note *literally states* about dosage/route/frequency/status | Yes | |
| Notices notes disagreeing with each other, without resolving it | Yes | |
| Normalizes medication names/fields for comparison (whitespace, casing, aliases) | | Yes (`app/services/medication_normalization.py`) |
| Decides whether an extracted mention conflicts with the patient's medication list | | Yes (`build_discrepancy_findings`, Decision 12) |
| Assigns a discrepancy's severity | | Yes (a fixed `SEVERITY_BY_DISCREPANCY_TYPE` mapping) |
| Attaches a mention to its true source document | | Yes (`source_note`, reported by AI, resolved deterministically) |
| Decides whether a discrepancy is later resolved/dismissed | | Yes (a human provider, via `POST .../resolve`; see `docs/api.md`) |

**Evidence generation.** Every `MedicationDiscrepancy` carries deterministic, template-generated `title`/`ai_explanation`/`expected_value`/`observed_value` fields (despite the field name `ai_explanation`, this text is generated by plain Python string formatting in `medication_reconciliation_service.py`, not by an AI call, a naming leftover from before the deterministic engine existed, not a description of how it currently works) and, on read, is enriched with the actual `MedicationMention`/`Medication` rows that produced it (see `docs/api.md`'s discrepancy-detail response), so a provider reviewing a finding sees exactly what evidence produced it, not just AI's summary of it.
...
OLLAMA_BASE_URL=http://localhost:11434   # used by AI_PROVIDER=openbiollm or AI_PROVIDER=medgemma
OPENBIOLLM_MODEL=openbiollm-llama3-instruct
MEDGEMMA_MODEL=hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M
```

Read via `app/core/config.py`'s `Settings` (a `pydantic-settings` `BaseSettings`, loaded from `.env`/the environment, with `extra="ignore"` so a stale variable an older version of this file used to read - e.g. a leftover `HUGGINGFACE_API_KEY` in an existing `.env` - is inert rather than a startup failure):
...
- **`OLLAMA_BASE_URL`** (`Settings.ollama_base_url: str = "http://localhost:11434"`): used by `AI_PROVIDER=openbiollm` and `AI_PROVIDER=medgemma`, both served by a local Ollama daemon rather than a hosted API - neither requires a credential of any kind. A connection detail, not a secret: the backend starts normally regardless of whether Ollama is actually reachable, and only a request that actually needs `OpenBioLLMProvider`/`MedGemmaProvider` fails, with a `503` and a message naming the problem (unreachable server, or model not installed - see Error Handling, below). Docker Compose (`infra/docker-compose.yml`) defaults this to `http://host.docker.internal:11434`, since Ollama is expected to run on the host, not as a container in that compose file; see `docs/deployment.md`.
- **`OPENBIOLLM_MODEL`** (`Settings.openbiollm_model: str = "openbiollm-llama3-instruct"`): which local Ollama model to call for OpenBioLLM - the locally-built, correctly-templated model, not the raw GGUF import (see Provider Abstraction, above, and Manual Setup, below).
- **`MEDGEMMA_MODEL`** (`Settings.medgemma_model: str = "hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M"`): which local Ollama model to call for MedGemma, pulled directly via Ollama's Hugging Face GGUF integration.
- **Request timeout**: `GeminiProvider.DEFAULT_TIMEOUT_MS = 30_000` (30 seconds), `OpenBioLLMProvider.DEFAULT_TIMEOUT_S = 120.0`, and `MedGemmaProvider.DEFAULT_TIMEOUT_S = 120.0`, each a hardcoded constant in its own provider module, **not** read from `Settings` or any environment variable. The local providers' timeout is deliberately much longer than Gemini's: local CPU/Metal-bound inference on consumer hardware is far slower than either hosted API, and 120s is a practical starting point for an 8B/4B Q4_K_M model on a single, non-streaming request, not a measured worst case.
- **Generation parameters** (OpenBioLLM and MedGemma only): both providers' `GENERATION_PARAMS` (`temperature=0`, `seed=0`, `num_predict=1024`, sent as Ollama's `options`) are hardcoded constants, not environment configuration; see Provider Abstraction, above, for why.
- **Production configuration**: no AI-specific settings differ between environments; every variable above is set the same way (`infra/.env`, see `docs/deployment.md`) in every environment, aside from `OLLAMA_BASE_URL`'s own Docker Compose default noted above. There is no separate "production model" or "production timeout."

---

## Manual Setup (OpenBioLLM/MedGemma)

Neither `openbiollm` nor `medgemma` works out of the box: both require a local [Ollama](https://ollama.com) installation with the right model pulled (and, for OpenBioLLM, a locally-built model with the correct chat template attached). This is manual, one-time, per-machine setup, not something the application or its test suite can do for you - the automated test suite never requires Ollama or any downloaded model weights (see Testing, below).

**1. Install Ollama and pull MedGemma** (used as-is, no custom Modelfile):

```bash
ollama pull hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M
```

**2. Pull the OpenBioLLM GGUF and Ollama's own reference Llama 3 Instruct model** (the latter only to read its canonical chat template):

```bash
ollama pull hf.co/aaditya/OpenBioLLM-Llama3-8B-GGUF:Q4_K_M
ollama pull llama3:8b-instruct-q4_K_M
```

**3. Build the locally-templated OpenBioLLM model** from the repo-tracked Modelfile:

```bash
ollama create openbiollm-llama3-instruct -f infra/ollama/openbiollm-llama3-instruct.Modelfile
```

**4. Verify the template attached correctly:**

```bash
ollama show openbiollm-llama3-instruct --modelfile
ollama list   # both the raw GGUF and openbiollm-llama3-instruct should be listed
```

**5. Smoke-test each model directly**, confirming it answers a direct extraction request instead of reformulating it as a question (the exact failure mode the untemplated GGUF import showed):

```bash
ollama run openbiollm-llama3-instruct "Extract the medication name from: patient takes lisinopril 10mg daily."
ollama run hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M "Extract the medication name from: patient takes lisinopril 10mg daily."
```

**6. Point the backend at Ollama and try a real request:**

```bash
# in backend/.env
AI_PROVIDER=openbiollm   # or: medgemma
```

then start the backend normally and call `POST /patients/{id}/analyses` (see `docs/api.md`) against a real synthetic document.

**7. Smoke-test through the real benchmark runner** (writes to `benchmark/results/`, gitignored; see `benchmark/README.md`):

```bash
python -m benchmark.runner --providers openbiollm medgemma --cases BENCH-001
```

Inspect the run's `manifest.json` for `"inference_backend": "ollama"` and the expected `model`/`generation_params`/`runtime_version` per provider, and `predictions.jsonl` for a real `provider_response`.
...
- **Ollama unreachable** (`OpenBioLLMProvider`/`MedGemmaProvider`, connection refused or DNS failure): `AIProviderError("Could not connect to Ollama at <base_url>. Is the Ollama server running?")`.
- **Ollama model not installed** (HTTP 404 from `/api/chat`): `AIProviderError("Ollama model '<model>' is not installed. Run: ollama pull <model>")`.
- **Ollama request timeout**: `AIProviderError("<OpenBioLLM|MedGemma> request timed out after <timeout_s>s")`, raised for both a bare `TimeoutError` and a `urllib.error.URLError` wrapping one.
...
AI is tested at three separate layers, each replacing a different real dependency with a fake, so the full test suite (`docs/testing.md`) never requires a live network call to Gemini, nor a running Ollama server or any downloaded model weights:

1. **Provider level** (`tests/test_gemini_provider.py`, `tests/test_openbiollm_provider.py`, `tests/test_medgemma_provider.py`): the underlying HTTP boundary is replaced via `monkeypatch` (`google.genai.Client` for Gemini; `urllib.request.urlopen` for OpenBioLLM/MedGemma), with small hand-written fakes that record what they were called with and return a scripted response or raise a scripted error, no mocking library, matching this project's general testing convention (`docs/testing.md`). Exercises each provider's own logic: the missing-credential case (Gemini only), successful text extraction, provider-specific request configuration (Gemini's JSON `response_mime_type`; OpenBioLLM's/MedGemma's `/api/chat` request shape, deterministic `options`, and prompt-to-`messages` wrapping), and error normalization to `AIProviderError` - including, for the local providers, a simulated connection refusal, a 404 model-not-found response, and a timeout. `test_openbiollm_provider.py` and `test_medgemma_provider.py` each additionally cover the shared cleanup step from `text_cleanup.py`: stripping a markdown fence, stripping surrounding prose, and, just as important, that malformed JSON is still malformed after cleanup, never repaired.
...
`OpenBioLLMProvider` and `MedGemmaProvider` are real instances of this section's steps, not just a hypothetical: both were added specifically to run the multi-provider benchmark comparison now documented in `docs/model-comparison-report.md` (#89-#91). To add another provider:
...
4. Add an `elif` branch to `build_ai_summary_service()` (`app/ai/service.py`) constructing the new provider, and extend `Settings._validate_ai_provider_configuration`'s allowed values (`app/core/config.py`) to include it.
...
- **Provider**: the intended extension point; adding one requires no change outside `app/ai/providers/`, `build_ai_summary_service()`, and `Settings`' own `AI_PROVIDER` validation.
- **Prompt template**: a single point of change (`SUMMARY_PROMPT_TEMPLATE`), but changing its *shape* also requires updating `ClinicalSummary`/`Medication` in lockstep, since nothing generates one from the other (a known trade-off; see Decision 16). Every provider receives the exact same prompt, unmodified; `build_summary_prompt()` has no provider-specific branching and none is planned.
...
- **Provider selection at runtime**: now a real, if narrow, extension point: `AI_PROVIDER` (see Configuration, above) selects one active provider per deployment. It is still not a per-request or multi-provider mechanism: a single deployment runs exactly one provider at a time, and #89's evaluation framework constructs specific provider classes directly (to run a fixed benchmark against several of them in one pass) rather than go through this single-active-provider setting at all.
...
- Three providers are implemented: Gemini (the default), OpenBioLLM, and MedGemma (`hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M`, a 4B checkpoint). OpenBioLLM/MedGemma were switched from Hugging Face's hosted Inference Providers to local Ollama inference before either had scored a single case, because both hosted paths proved operationally unreliable during pre-evaluation smoke testing; that switch is an infrastructure change, not a finding about either model. A full 30-case benchmark comparison across all three providers has since been run and scored (#89/#90); see `docs/model-evaluation.md` for the evaluation methodology and `docs/model-comparison-report.md` for the current results, including OpenBioLLM's investigated zero-evaluable-case outcome.
...
- Local inference latency (`OpenBioLLMProvider`/`MedGemmaProvider`) is not comparable to Gemini's hosted latency: both depend heavily on the evaluation machine's own hardware. `docs/model-comparison-report.md`'s Latency section reads `score_latency`'s per-provider breakdown accordingly, never as apples-to-apples across hosted and local providers.
