...
- Three providers are implemented: Gemini (the default), OpenBioLLM, and MedGemma (`hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M`, a 4B checkpoint). General provider benchmarking remains planned future work, behind the same `AIProvider` interface described above. No benchmark has been run against OpenBioLLM or MedGemma yet, and this document makes no claim about either's extraction accuracy relative to Gemini or to each other; that is #89-#91's work, not this one's. OpenBioLLM/MedGemma were switched from Hugging Face's hosted Inference Providers to local Ollama inference before either had scored a single case, because both hosted paths proved operationally unreliable during pre-evaluation smoke testing; that switch is an infrastructure change, not a finding about either model.
...
- Local inference latency (`OpenBioLLMProvider`/`MedGemmaProvider`) is not comparable to Gemini's hosted latency: both depend heavily on the evaluation machine's own hardware. A future benchmark comparison (#91) should not read `score_latency`'s per-provider breakdown as apples-to-apples across hosted and local providers.
