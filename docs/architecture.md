...
### Structured Logging (Issue #59)

Before this issue, the backend had no central logging configuration at all - four scattered `logging.getLogger(__name__)` loggers (AI service, Gemini provider, clinical document routes/service) ran on Python's unconfigured default root logger, no HTTP request was ever logged, and there was no way to correlate log lines from the same request. This issue replaces that with one centrally-configured structured logging system, without changing any of the standard-library `logging.getLogger(__name__)` calls elsewhere in the codebase (Decision 22, `docs/design-decisions.md`).
