...
PostgreSQL. Stores every persisted application resource: `User`, `Patient`, `ClinicalDocument`, `MedicationMention`, `Medication`, `Analysis`, `MedicationDiscrepancy`, `AnalysisMedicationMention`, and `AnalysisInconsistency`. It does **not** store uploaded files' original bytes - those go to the pluggable storage backend (local disk or S3); Postgres stores each document's *extracted text* (`raw_text`) and file metadata only. See Persistence (below) and `docs/data-model.md` for the full entity reference.
