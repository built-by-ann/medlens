...
- **AI summary service**: `AISummaryService` is tested against a fake in-memory provider rather than a live call, covering provider and model metadata on the result, the prompt reaching the provider, propagation of a provider error, and rejection of an empty note list.
- **Gemini provider**: the underlying `google.genai.Client` is mocked rather than called live, covering a missing API key failing before any client is constructed, a successful call returning the response text, translation of both an SDK API error and an unexpected exception into `AIProviderError`, rejection of an empty or missing response, and that the client is constructed once and reused across calls.
- **AI summarize endpoint**: authentication is required, a successful summary is returned for the caller's own documents using an overridden fake provider, multiple documents are combined into one prompt, a nonexistent or another user's document is rejected the same way as elsewhere in the API, an empty document id list is rejected, a provider failure surfaces as a `503`, and the real, non-mocked provider wiring fails gracefully with a `503` when no API key is configured in the test environment.
