...
The MedLens frontend is a React and TypeScript single-page application built with Vite. This document describes the application's infrastructure: project tooling, folder structure, routing, layout, the API client, and the authentication foundation, followed by each product feature area (patients, medications, clinical documents, analyses, settings, theming) in turn.
...
| `/upload` | redirects to `/patients` (legacy route; analyses and uploads are patient-scoped, see Layout below) | | |
| `/analyses/:id` | redirects to `/patients` (legacy route; analyses are patient-scoped, see Layout below) | | |
| `/` | redirects to `/dashboard` | | |
| `*` | `NotFoundPage` | none | no |

Login and registration are intentionally rendered outside `AppLayout`, since an unauthenticated user has no dashboard or upload links to navigate to.

Protected routes are wrapped in `ProtectedRoute` (`src/components/common/ProtectedRoute.tsx`), which reads `useAuth()` and redirects to `/login` when there is no authenticated user.
...
`TopNav` links to Dashboard and Patients, and shows either a "Log in" link or a "Log out" button depending on `useAuth()`'s `user` state. There is deliberately no standalone "Medications" or "Upload" link, since medication management, document upload, and analysis history all only exist within a patient's context (see Patients below), never as a global destination.

### Patient breadcrumb navigation

Every page nested under a patient (`PatientOverviewPage`, `UploadPage`, `PatientAnalysesPage`, `AnalysisDetailPage`, and others listed below) renders `PatientBreadcrumb` (`src/components/patients/PatientBreadcrumb.tsx`) above its `PageHeader`, so a provider can never lose track of which patient they're viewing or how to get back. It builds a `Patients / {patient name} / ...trail` `<nav aria-label="Breadcrumb">` with an `<ol>` of crumbs; every crumb is a link except the last, which is the current page and is rendered as plain text with `aria-current="page"` rather than a link to itself. Callers pass only the crumbs *after* the patient name (`trail`, e.g. `[{ label: 'Analyses', to: patientAnalysesPath(id) }, { label: 'Analysis #42' }]` for `AnalysisDetailPage`); `PatientOverviewPage` passes no trail at all, since the patient's own name is already the current page there.

Every patient-nested page uses a standardized Back action rather than each hand-rolling its own "← Back to {name}" link. Two shared components provide it:

- **`BackButton`** (`src/components/common/BackButton.tsx`): a single `<button>` rendering "← Back to {label}" (the arrow is `aria-hidden`, so the accessible name is exactly "Back to {label}"). Always navigates to the given logical-parent route (`to`): a fixed destination, not real browser-history back navigation. This is a generic, non-patient-specific component (`components/common/`, not `components/patients/`), even though every current caller happens to be patient-related.

  The fixed-destination design is deliberate. Real browser-history back navigation (`navigate(-1)`, falling back to `to` only when `useLocation().key === 'default'` indicated there was no in-app history to return to) was tried and rejected: the visible label always names the fixed logical parent (`to`/`label`), but `navigate(-1)` can land somewhere else entirely whenever the user's actual path diverges from a straight drill-down. For example, Overview -> Documents -> "View Patient" (back to Overview) -> clicking Overview's "Back to Patients" would silently land back on Documents instead, since that was the real previous history entry, while the label kept promising Patients. Since every patient-nested page already computes its own correct logical parent for `PatientPageNav`'s `backTo`/`backLabel` props (see the table below), and none of this app's pages depend on preserving scroll position or filter state across a real back-navigation, always honoring `to` keeps the label truthful on every click with no dependency on how the page was reached, at the cost of occasionally not landing on the literal previous page in edge cases like the one above, which is the correct tradeoff since the label is a promise about the destination.
- **`PatientPageNav`** (`src/components/patients/PatientPageNav.tsx`): the shared layout combining `PatientBreadcrumb` and `BackButton` into the one navigation block every patient-nested page renders identically: `<PatientPageNav patient={patient} trail={...} backTo={...} backLabel={...} />`. Bundling both in one component, rather than each page composing `PatientBreadcrumb` plus its own hand-rolled back link, is what eliminates duplicated navigation markup across patient-nested pages.

Every page nested under a patient uses `PatientPageNav` with a `backTo` matching its logical parent in the breadcrumb hierarchy:
...
| `CreateAnalysisPage` (renamed from `SelectDocumentsPage`) | `patientDetailPath(id)` | patient's full name |

`PatientDocumentsPage`'s existing "View Patient" action inside `<nav aria-label="Document actions">` is left as-is alongside `PatientPageNav`'s Back action: a purposeful action alongside Upload/Create Analysis, not a substitute for the standard top-of-page Back action every other patient page has.

**Deliberately out of scope for this pattern:**

- **`AnalysisProcessingPage`** keeps its plain `PatientBreadcrumb` with no `BackButton`. This page is a special-cased, transient, auto-submitting page (see Analysis Processing below) with its own explicit recovery links (`FailureCard`'s "Return to Patient Overview" / "Return to Analysis History", and the no-submission state's "Go to Upload") for exactly the situations where leaving the page makes sense; a generic Back action here would either duplicate those or invite navigating away mid-submission.
- **`NewPatientPage`** (`/patients/new`) has no existing patient yet, so `PatientPageNav`'s `patient`-shaped props don't apply; its bottom-of-form "Cancel" link (a distinct pattern also used by `EditPatientPage`) is used instead.
- **`PatientsPage`** (`/patients`) is the top of this hierarchy; the breadcrumb example is "Patients" alone, with nothing above it to add a Back action for.
...
- A request interceptor attaches the current auth token as an `Authorization: Bearer {token}` header whenever one is set. The token itself is held in a module-level variable in `client.ts`, not read from `AuthContext` directly, since interceptors run outside React and have no access to context; `AuthContext` remains the single owner of the token's value and calls `setAuthToken()` whenever it changes (login, logout, session restore), so this module never reads or writes storage itself.
...
Error handling follows one consolidated set of patterns applied consistently across the app: `ErrorState`, `Input`'s field-error wiring, and `useCreateAnalysis`'s upload-retry cache all follow the same conventions described below, rather than each surface inventing its own.

**`toApiError`** (`src/api/client.ts`, exported for direct testing, see `client.test.ts`) is the single place every backend failure passes through, and the one place worth understanding before anything else: it normalizes FastAPI's plain-string `detail`, list-of-field-errors `detail` (422s), and `{message, row_errors}`-shaped `detail` (the medication CSV importer) into one `ApiError` (`{status, message, detail?}`). It also special-cases the case with **no response at all** (offline, DNS/CORS failure, the backend down, a timeout), which would otherwise fall through to Axios's own `error.message` (literally `"Network Error"` or `"timeout of Xms exceeded"`, developer-facing text that would leak into every dialog, form, and card in the app verbatim). That case always returns `"Unable to reach the server. Check your connection and try again."` instead. No other status code is special-cased (a 403 is treated like any other error with a `detail` string); the backend's own messages are trusted and shown as-is everywhere except this one case where there is no backend message to show.
...
**Validation errors** are handled by `Input` (`aria-invalid`, `aria-describedby` pointing at a `role="alert"` `<p id={errorId}>`) everywhere a page uses it. `ManualNoteEditor` and `NoteCard`'s edit mode follow the same pattern rather than silently `disabled`-ing their Add/Save button when the note text is empty with no message explaining why: the button is always enabled, clicking it with empty text shows a `FormError` ("Note text is required.") wired via `aria-invalid`/`aria-describedby` exactly like an `Input` field error, and the button's own action (`handleAdd`/`saveEdit`) is what validates, not a `disabled` prop with no explanation. Typing anything clears the error immediately, matching the rest of the app's "input is preserved and re-validated live, not reset" convention.

**`FileDropzone`** (`src/components/upload/FileDropzone.tsx`) has an optional `errorId` prop, included in the dropzone's own `aria-describedby`, so an unsupported-file-type error (`useDocumentQueue`'s `fileError`) is linked back to the control that caused it rather than floating as a `FormError` paragraph elsewhere on the page, so a screen reader landing on the dropzone can tell why the last selection failed, matching how the medication CSV importer's dropzone already worked. `UploadPage` and `CreateAnalysisPage` pass a fixed id to both the prop and the paragraph's own `id`.

**Retry, and which item failed**: `useCreateAnalysis`'s upload loop caches each successfully-uploaded item by a stable queue id and skips re-uploading it on a subsequent `submit()`/`saveDocuments()` call (see "Retrying a partially failed submission" under Upload). It also computes `failedItemLabel` (the name of whichever file/note failed), which `UploadPage` and `AnalysisProcessingPage` compose into the displayed message so a multi-item failure is attributable to a specific item, not just "something failed": `` `${failedItemLabel}: ${error}` `` when known, falling back to the plain message otherwise.

`useAnalysisPolling` (`src/hooks/useAnalysisPolling.ts`) has a `retry()` function, following the same `retryCount`-as-effect-dependency pattern every other data-fetching hook in the app uses, so a single failed status-check request doesn't stop polling permanently with no way to resume short of leaving and re-entering the page. `AnalysisProcessingPage`'s `FailureCard` offers "Try again" for this case (calling `retryPolling`, which just re-checks status), distinct from its other "Try again" (calling `runSubmission`, which creates a new analysis); retrying a poll failure must never re-submit, since the original analysis may already exist and be fine.

`FailureCard` (`AnalysisProcessingPage.tsx`) uses the shared `Button` component directly for its retry action, rather than a hand-rolled `<button>` duplicating `Button`'s own styling, with `Button`'s missing `focus-visible` outline classes added back via `className` at this one call site. This is a gap in `Button` itself (it has no `focus-visible` styles built in, unlike almost every other interactive element in the app, which each add them inline) that remains out of scope to fix everywhere `Button` is used, but isn't worth reproducing further at this call site.

**Silent failures**: a background 401 (a request sent with a token that the backend no longer accepts, because the session expired or was revoked) triggers a silent logout, but the user isn't left with zero explanation. `AuthContext`/`AuthProvider` expose `sessionExpiredMessage: string | null` and `clearSessionExpiredMessage()`; the unauthorized-handler sets a message ("Your session has expired. Please log in again.") rather than just clearing state. `LoginPage` reads it once on mount, feeds it through the exact same `formError`/`FormError` mechanism every other login failure already uses (no separate banner or toast component), and clears it immediately so it can't reappear on a later visit.

---

## Authentication Foundation

- `AuthContext` (`src/contexts/AuthContext.ts`) defines the context and its value shape: `user`, `token`, `isAuthenticated`, `isLoading`, `login`, `logout`, and `sessionExpiredMessage`/`clearSessionExpiredMessage`; see Error Handling above.
- `AuthProvider` (`src/contexts/AuthProvider.tsx`) is the real implementation: `login()` calls the backend and persists the returned token (`src/lib/tokenStorage.ts`, localStorage); on mount, a stored token is restored and validated against `GET /users/me`, clearing it if invalid; a 401 on an already-authenticated request (registered via `setUnauthorizedHandler` in `api/client.ts`) is treated as the session ending and logs out silently apart from the message described in Error Handling above.
...
Analyses are scoped to a patient (see Layout above), so `DashboardPage` (`src/pages/DashboardPage.tsx`) is built around a different question than a global "recent activity" feed would answer: "what patient do I want to work on?" rather than "what analysis recently happened?" Analyses, documents, and medications are managed from within a patient's own pages (`PatientOverviewPage` and its children); the Dashboard leads into that workflow rather than duplicating it. It also surfaces a cross-patient Recent Analyses feed and a way to start an analysis without a patient already open (see below), backed by `CreateAnalysisPage` as the single, patient-scoped destination for starting an analysis and a small, purpose-built cross-patient analyses endpoint (see Backend below).

The Dashboard's layout, top to bottom, is:

1. **Welcome:** `PageHeader` with `Welcome back, {name}` (or a generic "Welcome back" when the user has no name).
2. **Quick Actions:** sits at the top of the page, in a `<nav aria-label="Quick actions">` with three entries: **+ New Analysis** (primary/blue, see below), **+ New patient** (`ROUTES.newPatient`), and **View all patients** (`ROUTES.patients`). Unlike every other section on this page, Quick Actions renders unconditionally, even in the "no patients yet" empty state, since it's meant to be reachable near the top of the Dashboard regardless of what else is going on; only **+ New Analysis** itself is disabled when there's no patient yet to start one for.
3. **Loading / error / empty states** (for the patient list specifically): `usePatients()` (the same hook `PatientsPage` uses) is the main data fetch driving this section. `LoadingSpinner`/`ErrorState` handle those states exactly as `PatientsPage` does; `EmptyPatientState` (`hasActivePatients={false}`) shows the same "No patients yet" onboarding CTA reused directly. The search box is hidden in this state (nothing to search yet), but Quick Actions is not.
4. **Patient search:** `PatientSearch` feeds the `filterPatients` utility (`components/patients/filterPatients.ts`, client-side, case-insensitive, over first name/last name/full name/MRN, since the backend's `GET /patients` has no search parameter). Search runs live on every keystroke and searches the *entire* loaded patient list, not just the Recent Patients preview below, so a patient outside the top 3 is still found.
...
6. **Recent Analyses:** see its own subsection below. Renders independently of the patient-list state above it (it has its own loading/error/empty states, from its own hook), including in the "no patients yet" case, where it shows its own empty message rather than being hidden.

Items 4-6 (patient search + Recent patients/Search results, and Recent Analyses) sit in a two-column CSS grid (`grid-cols-1 lg:grid-cols-2`): a single column on phone and tablet portrait, patients on the left and analyses on the right from `lg:` (1024px) up. This is a purely visual rearrangement: the DOM order is unchanged (patients column first, analyses column second), so tab order and screen-reader reading order match the stacked mobile layout exactly; there's no separate "desktop navigation path" to keep in sync.

### Starting an analysis from the Dashboard
...
The picker reuses `PatientSearch` and `filterPatients` directly, rather than a separate search implementation, over the full `patients` array `usePatients()` already loaded on the page, the same list Recent Patients and the main search box already work from, not a second fetch. Each matching patient renders as a plain `<button>` (name, MRN if present), fully keyboard-operable (Tab, Enter/Space) without needing a second component like `PatientCard`, which carries View/Edit/Archive actions this picker has no use for.

### Recent Analyses

A cross-patient feed, unlike everything else `DashboardPage` shows: the one place in the whole app an analysis is displayed outside its own patient's pages. Backed by `useRecentAnalyses(limit = 3)` (`src/hooks/useRecentAnalyses.ts`), calling `getRecentAnalyses(limit)` (`src/api/analyses.ts`), against a dedicated backend endpoint (see Backend below). The hook mirrors `usePatientAnalyses`'s shape (`{ analyses, isLoading, error, retry }`) minus `removeAnalysis`: this feed is read-only, since deleting an analysis remains a patient-page action (`AnalysisDetailPage`'s own delete workflow, see "Deleting an analysis from the Analysis Results page" under Analyses below), not something this glance-and-go preview offers.

Each analysis renders with `RecentAnalysisCard`, the exact same component `PatientOverviewPage`/`PatientAnalysesPage` use, with an optional prop, `patientName?: string` (default unset, so every other caller's rendering is unaffected). When present, it renders the patient's name above the status badge, and folds into the link's own `aria-label` ("View analysis for {patientName} from {date}, status: {status}"), so a screen reader user gets the same "which patient is this" context sighted users get from the visible name. `DashboardPage` passes it as `` `${analysis.patient.first_name} ${analysis.patient.last_name}` ``, using the `patient` object the recent-analyses endpoint nests on each row; no other caller passes it at all, since they're already on that patient's own page.

### Backend: a cross-patient analyses endpoint

Every other analyses endpoint is nested under `/patients/{patient_id}/analyses`. Recent Analyses is the only thing in this app that needs analyses spanning *every* patient a user owns, and it's backed by a deliberately narrow, dedicated endpoint: `GET /analyses/recent` (see `docs/api.md`), scoped to the current user via the same `get_current_user` dependency every other endpoint uses, not a `/me`-style prefix, matching how `GET /patients` is already implicitly "my patients" with no user id in the URL.
...
- **Service**: `list_recent_analyses_for_user(db, user_id, limit)` (`app/services/analysis_service.py`) joins `Analysis` to `Patient` to filter by `Patient.user_id` and exclude archived patients (`Patient.status != ARCHIVED_STATUS`, the same constant and exclusion `list_patients` applies), ordered by `Analysis.id.desc()`, the same tie-breaking reasoning as every other analyses listing in this codebase (Postgres's `now()` is constant within a transaction, so `created_at` alone can't be trusted to break ties).
- **Schema**: `RecentAnalysisResponse` (`app/schemas/analysis.py`) extends `AnalysisSummaryResponse` with one field, `patient: PatientSummaryResponse`, a minimal schema (`app/schemas/patient.py`: `id`, `first_name`, `last_name`), the same "citation, not the full resource" shape `ClinicalDocumentSummaryResponse` establishes for nesting evidence elsewhere.

No migration, no new table, no change to any other endpoint's behavior; this is a dedicated query and response shape over data that already existed.

### Recent patient strategy

Patient access is not tracked anywhere in this app (no "last opened by this provider" timestamp), so the ideal "recently accessed" ordering isn't available. `sortPatientsByRecentActivity` (`components/patients/sortPatientsByRecentActivity.ts`, a pure function alongside `filterPatients`) instead sorts by `updated_at ?? created_at` descending: a patient's most recent edit if it's ever been edited (`Patient.updated_at` is set on `PATCH`, `app/services/patient_service.py`), falling back to its creation time if it hasn't. This follows a fallback chain of access timestamps, then recently updated, then creation date descending, using the two signals that actually exist (access timestamps aren't available, see above). The result is sliced to `RECENT_PATIENTS_LIMIT` (3), a glance-and-go preview, not the full history that `PatientsPage` already provides.

`PatientCard` has two optional props to show what this preview needs beyond what `PatientsPage`'s own list shows: `showStatus` and `showUpdatedAt` (both default `false`, so `PatientsPage`'s own cards render without them). `DashboardPage` passes both as `true`; `updated_at` being `null` for a never-edited patient simply omits that one stat rather than showing an empty value. `PatientList` threads both through to every `PatientCard` it renders. This is the same card everywhere, reusing `PatientCard`/`PatientList` for the Dashboard rather than building a second, near-identical "recent patient" card design.

`patientStatusLabel` (`src/utils/patientStatus.ts`) holds the `{ active: 'Active', archived: 'Archived' }` lookup as a shared utility, so `PatientCard`'s status display and `PatientDetails` use the identical mapping instead of two copies.

---

## Upload

`UploadPage` (`src/pages/UploadPage.tsx`) is nested under a patient: `/patients/:patientId/upload`. It fetches the patient via `usePatient` (the same hook `PatientOverviewPage` and `PatientMedicationsPage` use) to show the patient's name in the heading, a breadcrumb, and a back link, then lets the user supply one or more clinical notes for that patient, either as files or pasted text.

**`UploadPage` only saves documents; it does not start an analysis.** There is no "Start Analysis" action here; **Save documents** is the page's only action. Analysis creation is unified onto one page, `CreateAnalysisPage` (see below); keeping a second, independent "upload and analyze in one step" path on `UploadPage` would recreate the separate upload and existing-document workflows that `CreateAnalysisPage` exists to avoid. `UploadPage` is purely a document-management utility: add files/notes, click Save documents, land back on `PatientDocumentsPage` with them already on file. Building an analysis from them (alone or combined with anything else already on file) happens on `CreateAnalysisPage` afterward, whenever the provider is ready.
...
`useDocumentQueue.handleFilesSelected` defaults a queued CSV file's document type to Medication list (`DEFAULT_CSV_DOCUMENT_TYPE`) rather than the usual Visit note default, since a CSV in this app is overwhelmingly a medication list, so this saves a manual dropdown change in the common case; it's still freely changeable via the same `DocumentTypeSelect` every other file uses. A CSV is otherwise indistinguishable in the UI from any other queued file; its filename (with `.csv` extension) is enough to identify it before upload, and `ClinicalDocumentCard`'s `FILE_TYPE_LABELS` map (see Clinical Documents under Patients below) has a `csv: "Uploaded .csv file"` entry so it's identified the same text-based way `.txt`/`.pdf` uploads are, once saved.

### Retrying a partially failed submission

`useCreateAnalysis` caches each item's resulting document id (`fileItemKey(id)`/`noteItemKey(id)` to a `Map`, held in a ref) as soon as it uploads successfully. If a later item then fails, calling `submit()` again with the same queue skips re-uploading whatever already succeeded and only retries what didn't, rather than creating duplicate ClinicalDocument rows. The cache lives inside the hook, not in the caller's own state, since nothing outside a submission attempt needs to read it. The cache is also cleared automatically the moment an analysis is actually created, since any submission after that is a new attempt, not a retry. `failedItemLabel` (the failing file's name, or the note's title/fallback) is exposed alongside the error message so a multi-item failure is attributable to a specific item, not just "something failed"; `UploadPage` and `AnalysisProcessingPage` compose it into the displayed message (`` `${failedItemLabel}: ${error}` ``). `invalidateItem` and this retry cache live entirely on `AnalysisProcessingPage` (see below) rather than `UploadPage`: editing a file or note only ever happens *before* any submission attempt exists, so there is nothing yet to invalidate on the upload page itself; the "Try again" action on the processing page is what re-runs `submit()` against the same queue and benefits from the cache.

### Saving documents without starting an analysis

The backend has always treated document creation and analysis creation as two independent operations: `POST /patients/{patientId}/clinical-documents*` vs. `POST /patients/{patientId}/analyses`. The frontend mirrors that split: **Save documents** is `UploadPage`'s only action, styled blue/primary as the page's sole call to action, with no "Start Analysis" action alongside it (see the opening paragraph above).
...
The file/note queueing itself (state, file-type validation, de-duplication, edit, remove) lives in a shared hook, `useDocumentQueue` (`src/hooks/useDocumentQueue.ts`), since `CreateAnalysisPage`'s "Upload Additional Documents" section needs to behave identically to `UploadPage`'s own upload section. Both pages call this one hook rather than maintaining separate, could-drift-apart copies of the same logic; `isDuplicateFile` (same-name-and-size de-duplication) lives separately in `src/utils/queuedFiles.ts` for the same reason. Neither page calls any upload API directly from this state; the hook only manages the local queue; `saveDocuments`/`submit` (both on `useCreateAnalysis`) are what actually persist it.

---

## Create Analysis

`CreateAnalysisPage` (`src/pages/CreateAnalysisPage.tsx`, `/patients/:patientId/analyses/new`; renamed from `SelectDocumentsPage.tsx` at `/analyses/select-documents`, worth knowing when grepping the codebase for the old name) is the single entry point into analysis creation: a provider can select existing documents, upload new ones, and combine both into the same analysis, all on one page, rather than being forced to pick one workflow or the other. No backend change was needed to support this: `POST /patients/{patientId}/analyses` (`create_analysis`, `app/services/analysis_service.py`) has never distinguished a newly uploaded document from one already on file; it only validates that every requested id belongs to the given patient, so combining both kinds of ids in one request already works. See `docs/api.md`.
...
- **Upload Additional Documents:** `FileDropzone` and `ManualNoteEditor`, the exact same components `UploadPage` uses, both driven by the same shared `useDocumentQueue()` hook (see Upload above). A file or note added here is immediately reflected in the Selected Documents count below and in the page's overall selection; there is no separate "confirm upload" step, and an uploaded document becomes part of the current analysis selection immediately.
...
Clicking Create Analysis does **not** call `useCreateAnalysis` itself on this page. It navigates to `analysisProcessingPath(patientId)` with `{ files, notes, existingDocumentIds: Array.from(selectedExistingIds) }` as router state, the `SubmitInput` shape `AnalysisProcessingPage` (see below) handles; `useCreateAnalysis`'s `submit()` combines uploaded and existing ids in one `createAnalysisFromDocuments` call.

The entry point lives in three places: `PatientOverviewPage`'s Quick Actions ("Create Analysis," primary/blue, see "Quick Actions and the 'Create Analysis' question" under Patients), `PatientAnalysesPage`'s "+ Start analysis" action, and `PatientDocumentsPage`'s "Create Analysis" action (`createAnalysisPath`); all three point here.

### Searching and paging Existing Documents

A patient with a long document history would otherwise turn the plain checkbox list into content overload, which the unified page is meant to avoid. The Existing Documents section adds, above the checklist:
...
- `useAnalysisPolling` itself returns an `error` (the status-check request failed, independent of the analysis's own status). It also gets a "Try again" (via the hook's own `retry()`), but this one only re-checks status rather than resubmitting: resubmitting would create a duplicate analysis, since the original one may already exist and be fine. The user can also go to Analysis History instead, where the analysis (whatever its real status turns out to be) will already be listed.

---

## Analyses

Analysis history is scoped to a patient: `PatientAnalysesPage` (`src/pages/PatientAnalysesPage.tsx`, `/patients/:patientId/analyses`) shows the full history for one patient, not a global feed (see Dashboard above for the separate cross-patient Recent Analyses preview). It fetches the patient via `usePatient` (for the heading, breadcrumb, and back link) and the patient's analyses via `usePatientAnalyses(patientId, 50)` (`src/hooks/usePatientAnalyses.ts`); this page passes 50, the backend's maximum, rather than the hook's default of 10, since this page is the full browsable history, not a preview. The hook calls `listAnalyses(patientId, limit)` (`src/api/analyses.ts`, `GET /patients/{patientId}/analyses`) and exposes `{ analyses, isLoading, error, retry, removeAnalysis }`, the same shape `usePatientMedications` and the other patient-scoped hooks use; `removeAnalysis` calls `deleteAnalysis(patientId, id)` (`DELETE /patients/{patientId}/analyses/{analysisId}`) and removes the row from local state on success.

`src/components/analyses/`:

- `RecentAnalysisCard`: one analysis, with a status badge (`AnalysisStatusBadge`, so `AnalysisDetailPage` can reuse the exact same label/color mapping instead of a second copy), created/completed timestamps, the AI summary text, document count, and finding counts via `SummaryStat` (`components/common/SummaryStat`), provider/model if present. The whole card is a single `Link` to `analysisDetailPath(analysis.patient_id, analysis.id)` (using the `patient_id` present on `AnalysisSummary` itself, rather than a prop threaded down from the page), with an explicit `aria-label` describing the date and status. An optional `onDelete` prop renders a "Delete" button in a separate footer row *outside* the `Link` (a button nested inside an anchor would be invalid, keyboard-unreachable markup). `PatientAnalysesPage` passes `removeAnalysis`; `PatientOverviewPage`'s Recent Analyses preview does not, since delete belongs to the full history page, not the glance-and-go preview.
...
- `AnalysisStatusBadge`: the status-to-label/color mapping, extracted out of `RecentAnalysisCard` so `AnalysisDetailPage` renders an identical badge rather than a near-duplicate. The label lookup itself (`analysisStatusLabel`) lives in `src/utils/analysisStatus.ts`, not this component file: keeping a plain function out of a component file avoids tripping the `react-refresh/only-export-components` lint rule.
- `DeleteAnalysisDialog`: the confirmation dialog `AnalysisDetailPage`'s "Delete analysis" action opens; see "Deleting an analysis from the Analysis Results page" below.

`AnalysisDetailPage` (`src/pages/AnalysisDetailPage.tsx`, `/patients/:patientId/analyses/:analysisId`) fetches real data via a `useAnalysisDetail(patientId, analysisId)` hook (`src/hooks/useAnalysisDetail.ts`) calling `getAnalysisDetail` (`api/analyses.ts`, `GET /patients/{patientId}/analyses/{analysisId}`). It shows a breadcrumb (Patients / patient name / Analyses / Analysis #N), the same `AnalysisStatusBadge`, the sanitized `error_message` for a failed run, the `summary` text, and whatever of `created_at`/`started_at`/`completed_at`/`provider`/`model_name` the backend actually populated (an item is omitted rather than shown as empty when its value is `null`). Finding/severity counts are deliberately not shown here; they're only on `AnalysisSummaryResponse`, not the detail response (a document count *is* included in the detail response too, see below).

`medication_mentions` and `possible_inconsistencies` (the AI's raw, unstructured observations) are rendered in the AI Summary section described below; the frontend's `AnalysisDetail` type (`types/api.ts`) exposes both fields for that purpose.

The medication reconciliation engine feeds into analysis creation (see `docs/architecture.md`'s Analysis Creation Pipeline), so completed analyses can have real `MedicationDiscrepancy` findings; `AnalysisDetail` has a `medication_discrepancies` field. The presentation of those findings:
...
- **Grouping and sorting**: `groupBySeverity` (`AnalysisDetailPage.tsx`) buckets findings into high/medium/low groups, in that order, dropping empty groups; within a group, findings are sorted by ascending `id` (stable, matching the order the backend already returns them in). Each non-empty group renders under its own `<h3>` ("High severity (2)"). There is no "critical" tier: `DiscrepancySeverity` (the backend enum) only has `high`/`medium`/`low`, so the frontend reuses the existing enum rather than inventing a 4th tier that would require a backend change.
- **`MedicationDiscrepancyCard`** (`components/analyses/MedicationDiscrepancyCard.tsx`): the reusable card, one per finding: medication name (falling back to "Unknown medication" if neither piece of evidence below is present, a purely defensive case the real reconciliation engine never actually produces, since every finding type sets at least one), `discrepancyTypeLabel` (discrepancy type), `DiscrepancySeverityBadge` and `ResolutionStatusBadge` (both `components/analyses/`), `ai_explanation`, an `expected_value`/`observed_value` pair via `SummaryStat` when present, `recommendation` when present, and a "Supporting evidence" subsection: the mention's source document + `context_text` snippet when `medication_mention` is present, a one-line current-medication-list summary when only `medication` is present, or an explicit "No supporting evidence was recorded for this finding." when neither is, never a blank gap. A colored left border repeats the severity badge's signal, since severity is never conveyed by color alone.
- **`DiscrepancySeverityBadge`** / **`ResolutionStatusBadge`**: the same visible-text-label-always pattern `AnalysisStatusBadge` establishes; label lookups live in `utils/discrepancy.ts`, not the component files, for the same `react-refresh/only-export-components` reason `analysisStatusLabel` does. `ResolutionStatusBadge`'s label for `reviewed` is shown as "In Review" even though the underlying enum value is unchanged.
- **Empty state**: a positive `Card` ("No medication inconsistencies were detected."), shown when `medication_discrepancies` is empty.
...
Backend: `MedicationDiscrepancyResponse` (used inside `medication_discrepancies`) carries nested `medication`/`medication_mention` evidence; see docs/api.md and docs/data-model.md.

The top of `AnalysisDetailPage` is a dedicated "AI Summary" section, keeping the AI's raw output distinct from the deterministic reconciliation findings below it:
...
- **Summary**: an `<h3>` rendering `analysis.summary`, or "No AI summary is available for this analysis." when it's `null`. Reconciliation findings below are unaffected either way; a missing AI summary never hides them.
- **Medications Mentioned**: an `<h3>` rendering `medication_mentions` as a compact list of name / dosage / route / frequency / status / notes. The heading and list are omitted entirely when the array is empty; no placeholder content.
- **Summary Metadata**: a `<h3>` labeled sub-block reusing `SummaryStat` for `created_at`/`started_at`/`completed_at`/`provider`/`model_name` (each omitted when `null`) plus `document_count`, labeled "Documents analyzed".
- **No "Key clinical observations", "Mentioned conditions", or "Notable findings" sections.** `app/ai/schemas.py`'s `ClinicalSummary` has only `summary`, `medications`, and `possible_inconsistencies`; no structured field corresponds to conditions or "notable findings" as a concept distinct from the summary text itself. Rather than fabricate content or repurpose an existing field under a misleading heading, these sections are simply not present, the same "never invent data" principle behind not inventing a "critical" severity tier above.
- **"Deterministic" badge**: next to "Medication Reconciliation Findings" (`DeterministicBadge`, same page-local pattern as `AiGeneratedBadge`), so the AI/deterministic distinction reads at both ends of the page, not just at the top.
- **Heading hierarchy**: h1 (page) > h2 ("AI Summary") > h3 (Summary / Medications Mentioned / Summary Metadata) > h2 ("Medication Reconciliation Findings") > h3 (severity group) > h4 (medication name) > h5 ("Supporting evidence"), still non-skipping; the two h2 sections are siblings, not nested.

Backend: `Analysis` has a `document_count` property (`len(self.clinical_documents)`, the same computed-property pattern as `ClinicalDocument.analysis_count`), exposed as a field on `AnalysisDetailResponse`; `get_analysis_for_patient` uses `selectinload(Analysis.clinical_documents)` to compute it without an N+1 query. See docs/api.md and docs/data-model.md.

**There is no "Follow-up Questions" section rendering `possible_inconsistencies` as a checklist.** Such a section existed at one point and was removed for two compounding reasons: the heading promised questions, but `possible_inconsistencies` is the AI's raw description text, unstructured observations, never phrased as a question, so the label never matched the content it introduced. And the checkbox itself implied real state (the same visual language `MedicationDiscrepancyCard`'s resolution actions use for something that actually persists) while the old checklist's checked state was `useState` only, silently reset on every reload, with nothing behind it. Once the reconciliation workflow gave this page a second, real actionable checklist with genuine persisted state (resolve/dismiss), the contrast made the fake one worse, not better: a control that looks actionable but isn't undermines trust in the one next to it that is. `possible_inconsistencies` and `AnalysisInconsistency` are untouched on the backend and in `types/api.ts`; only the frontend's rendering of this one field was removed; reviving it (with an accurate heading, and either no checkbox or a checkbox backed by a real persisted field) remains possible without any backend change.

### Deleting an analysis from the Analysis Results page

`DELETE /patients/{patient_id}/analyses/{analysis_id}` is used from two frontend entry points: `PatientAnalysesPage`'s inline list delete (`deleteAnalysis`/`removeAnalysis`, described above) and `AnalysisDetailPage`'s own delete action, described here, which adds a confirmation step the list page's inline delete doesn't have.

- **`DeleteAnalysisDialog`** (`src/components/analyses/DeleteAnalysisDialog.tsx`): a dialog built on the same native `<dialog>` pattern `ArchivePatientDialog` uses (see Patients > Archiving below): `showModal()`/`close()` for the overlay, backdrop-click-to-cancel, and `onClose` as the single place that syncs React state back to "closed." Its `target: { id, patientName } | null` prop is both the dialog's content and its open/closed flag, the same shape `ArchivePatientDialog`'s `patient` prop plays. Shown: "Analysis #{id} for {patientName}" plus permanent-deletion wording.
...
- **The final-analysis empty state needs no special handling.** `PatientAnalysesPage` re-mounts `usePatientAnalyses` fresh on navigation and re-fetches from the server, so if the deleted analysis was the patient's last one, `AnalysesEmptyState` renders automatically.

**The app's only toast-style notification.** No general toast/notification component exists elsewhere in this codebase (see Patients > Archiving below); every other delete/remove action's only feedback is the immediate UI change itself (a card disappearing, a redirect). A temporary success notification is needed here specifically because the confirming page (`AnalysisDetailPage`) is gone by the time the user lands on `PatientAnalysesPage`. Rather than build a general-purpose toast system for a single caller, `PatientAnalysesPage` reads an optional `flashMessage` off `useLocation().state` in a `useEffect`, shows it in a dismissible `role="status"` banner, and immediately replaces the location's state with `null` (`navigate(location.pathname, { replace: true, state: null })`) so refreshing or navigating back to that history entry can't redisplay a notification for an action that already happened. It also auto-clears after 6 seconds (`FLASH_MESSAGE_DURATION_MS`) via a plain `setTimeout` effect. If a second caller ever needs the same pattern, this is the point to extract a shared hook; one caller doesn't justify that abstraction yet.

### Resolving discrepancies from the Analysis Results page

`MedicationDiscrepancyCard` (above) renders a discrepancy for review and also lets a provider act on a finding directly, extending the same card and page rather than introducing a second results view.

**Which actions a card offers is data-driven, not hardcoded per discrepancy type.** `getDiscrepancyActions(discrepancy)` (`src/utils/discrepancyResolutionActions.ts`) is a pure function mapping a discrepancy's `discrepancy_type` (and, for the accept actions, its nested `medication`/`medication_mention` evidence) to an ordered list of `DiscrepancyActionDescriptor`s, each carrying a `label` ("Add Medication", "Update Medication", "Mark Discontinued", "Mark Active", "Edit Manually", "Dismiss"), the underlying `ResolutionAction` it submits (`add_medication`/`update_medication`/`dismiss`, see `docs/api.md`), whether its fields are user-editable, and pre-filled `fields` sourced from the discrepancy's own evidence:

| `discrepancy_type` | Actions offered, in order |
|---|---|
| `missing_from_medication_list` | Add Medication (pre-filled from `medication_mention`, all 5 fields editable and required), Dismiss |
| `dose_conflict` / `route_conflict` / `frequency_conflict` | Update Medication (the one conflicting field, pre-filled from the mention's value, not editable: a one-click accept), Edit Manually (all 5 fields, pre-filled from the current `medication`, editable), Dismiss |
| `discontinued_status_conflict` / `status_conflict` | Mark Discontinued, Mark Active (both a fixed, non-editable `status` field: "Mark X" is `update_medication` with `status` already set to X, not a distinct backend action; see `docs/architecture.md`'s Discrepancy Resolution Workflow), Edit Manually, Dismiss |
| `unsupported_medication_list_entry` | Edit Manually, Dismiss |

This is what keeps a `MedicationDiscrepancyCard` from exposing actions that don't make sense for a given discrepancy: it never special-cases a discrepancy type in its own render logic to decide which buttons to show; it always renders exactly what `getDiscrepancyActions` returns, so adding or changing an action mapping never touches the card component. `Update Medication`'s single-field variant is additionally omitted when the mention never actually reported that field (e.g. a `dose_conflict` where `medication_mention.dose` is `null`), so there is nothing to one-click accept and only Edit Manually and Dismiss remain. `requiredFieldKeys`/`buildResolutionPayload` (same file) complete the mapping: which of an action's fields must be non-empty before submission, and how to turn the dialog's current field values plus an optional note into the exact `DiscrepancyResolutionPayload` body `POST .../resolve` expects (trimmed, empty fields omitted, per `docs/api.md`'s request shape).

**`ResolveDiscrepancyDialog`** (`src/components/analyses/ResolveDiscrepancyDialog.tsx`) is the one dialog every action opens, editable or not, built on the same native `<dialog>` pattern `DeleteAnalysisDialog`/`ArchivePatientDialog`/`StartAnalysisDialog` already established (`showModal()`/`close()`, backdrop-click-to-cancel, a persistent `<dialog>` with `{target && (...)}` conditional content rather than two different dialog shapes). Its `target: ResolveDiscrepancyTarget | null` prop carries the chosen `DiscrepancyActionDescriptor` directly, so the dialog needs no discrepancy-type-specific branching either: when `action.editable` is true, it renders one labeled text input per `action.fields` entry (marked `*` and required when `requiredFieldKeys` includes it), pre-filled from the descriptor's own suggested values; when false (a one-click accept like "Update Medication" for a single-field conflict, or "Mark Discontinued"/"Mark Active"), the fields are hidden entirely and the dialog is just a confirmation with its `action.description` explaining what will happen. Every action's dialog also offers an optional "Note" textarea, submitted as `resolution_note` regardless of action, so provider notes are supported uniformly rather than only for some actions. `Dismiss` opens the same dialog with no fields, since it's a destructive action with no undo and gets the same confirmation-dialog treatment as every other destructive action (see Accessibility below).

**Wiring, on `AnalysisDetailPage`**: `MedicationDiscrepancyCard` has an optional `onResolveAction` prop; when provided and the discrepancy is still `open`, it renders the action buttons described above, each calling back with `{ discrepancyId, medicationName, action }`. `AnalysisDetailPage` uses that callback to open `ResolveDiscrepancyDialog`, and on confirm calls `resolveDiscrepancy(patientId, analysisId, discrepancyId, payload)` (`src/api/analyses.ts`, `POST .../resolve`). On success:

- `useAnalysisDetail`'s `replaceDiscrepancy(updated)` splices the returned, fully-resolved `MedicationDiscrepancy` into `analysis.medication_discrepancies` in place (matched by `id`), so the card immediately re-renders with its resolved state, with no full analysis refetch and no risk of a second discrepancy's own concurrent edit being clobbered by a stale refetch response.
- A `role="status"` success message ("Lisinopril added to the medication list." / "Discrepancy dismissed.") is shown briefly, so the status update is announced to assistive technology as well as shown visually.
- Because `MedicationDiscrepancyCard` derives its own severity summary counts and grouping from the same `medication_discrepancies` array `AnalysisDetailPage` already holds, the Findings summary (Total/High/Medium/Low) and severity grouping stay correct automatically: resolving a finding never removes it from the array (dismissed and resolved findings both remain visible, clearly marked), only its own `resolution_status`/audit fields change.
- `RecentAnalysisCard`'s "Open" stat (backed by the `open_findings` field, see `docs/api.md` and `docs/architecture.md`) reflects the change the next time that card's own data is fetched (Patient Overview's Recent Analyses preview, the Dashboard's cross-patient feed); both already re-fetch on mount, so no additional cache-invalidation code is needed; this app has no client-side cache to invalidate (see State Management below).

**Resolved-state rendering.** Once `resolution_status` is no longer `open`, `MedicationDiscrepancyCard` stops rendering any action button for that card at all (editing a resolved finding is not possible, and there is no separate `disabled` styling to design, since the buttons are simply absent) and instead renders an audit block: "Resolved by {name or email} on {formatted resolved_at}" or "Dismissed by ..." (label depends on `resolution_status`), plus the `resolution_note` when present. `ResolutionStatusBadge` already visually distinguishes Open/Resolved/Dismissed with both color and a text label, never color alone, so a resolved or dismissed card is clearly marked at a glance in addition to the detailed audit line below it.

**Accessibility**, matching this codebase's established patterns (see Accessibility below):

- Every action button's accessible name is the full action plus medication (`aria-label="Update Medication: Lisinopril"`), not just the visible label, so a screen reader user moving between several cards' buttons doesn't have to guess which medication a bare "Dismiss" applies to.
- `Dismiss` (the one action with no undo) opens the same confirmation dialog every other action does, rather than acting immediately on click.
- On open, `ResolveDiscrepancyDialog` focuses its Cancel button first (the same explicit `cancelButtonRef.current?.focus()` pattern `DeleteAnalysisDialog` uses, since this project's jsdom `<dialog>` polyfill doesn't replicate the browser's own auto-focus-first-control behavior; see "Deleting an analysis from the Analysis Results page" above), landing keyboard/screen-reader focus on the safest action by default.
- The success message uses `role="status"` (not `role="alert"`), matching `LoadingSpinner`'s existing convention for non-error status changes, announced without interrupting whatever the user is doing, unlike an alert.

---

## Patients

Patient management is built directly against the Patient CRUD API (`docs/api.md`'s `/patients` endpoints).

Five routes, five pages:

- `PatientsPage` (`/patients`): search, a "+ New patient" action, and the full active patient list. `usePatients` (`src/hooks/usePatients.ts`) fetches the list on mount (`{ patients, isLoading, error, retry, archivePatient }`, the same shape `usePatientMedications`/`usePatientAnalyses`/`usePatientClinicalDocuments` all use) and exposes `archivePatient`, which updates local state directly on success rather than refetching. `DashboardPage` calls this exact same hook (see Dashboard above) rather than adding a second patient-fetching hook or duplicating the request.
...
- `PatientOverviewPage` (`/patients/:patientId`): the patient's central workspace. Top to bottom: a breadcrumb, identity/demographic display (`PatientDetails`) with Edit/Archive actions, a Quick Actions bar ("Upload document", "Manage medications"), a Clinical Documents section, a Recent Analyses preview, and the Medications section (see Medications below; see Clinical Documents below and Analyses above for those two sections). `usePatient(patientId)` (`src/hooks/usePatient.ts`) fetches the single record; a 404 for a nonexistent or not-owned patient surfaces through the normal `error` state; there's no separate "not found" UI, since the backend's `"Patient not found"` detail already reads correctly as an error message.
...
- `sortPatientsByRecentActivity.ts`: the "recent patients" sort; see "Recent patient strategy" under Dashboard above.
...
- `ArchivePatientDialog`: built on the native `<dialog>` element (`showModal()`/`close()`) rather than a hand-rolled overlay; see Accessibility below.
- `PatientBreadcrumb` / `PatientPageNav`: see Patient breadcrumb navigation, above.

### Clinical Documents

`usePatientClinicalDocuments(patientId)` (`src/hooks/usePatientClinicalDocuments.ts`) fetches the patient's documents via `listClinicalDocuments` (`api/clinicalDocuments.ts`, `GET /patients/{patientId}/clinical-documents`) and exposes `{ documents, isLoading, error, retry, removeDocument }`, the same shape as the other patient-scoped hooks; `removeDocument` calls `deleteClinicalDocument` (`DELETE /patients/{patientId}/clinical-documents/{documentId}`).

Clinical documents have two distinct views, matching how Analyses works (a glance-and-go preview on Overview, a full browsable history on its own page):

- **Recent Clinical Documents** (`PatientOverviewPage`): a compact preview, the 3 most recent documents (`documents.slice(0, RECENT_DOCUMENTS_PREVIEW_LIMIT)`), each a `RecentDocumentCard`/`RecentDocumentsList` (`src/components/documents/`) showing title/type/date, clickable to expand inline and read the document's extracted text (the same interaction `ClinicalDocumentCard`'s "View" offers, just without a separate Delete action here, see below). The backend's `GET /patients/{patientId}/clinical-documents` has no `limit` query parameter (unlike analyses' `?limit=`), so the 3-most-recent cut is applied client-side against the same full list the hook already fetches, rather than a backend change; see Backend below. A "View All" link to `patientDocumentsPath(patientId)` appears next to the section heading, but only once there is at least one document to view (an empty patient shows `EmptyDocumentsState` instead, with nothing to page through).
- **Clinical Documents page** (`PatientDocumentsPage`, `src/pages/PatientDocumentsPage.tsx`, `/patients/:patientId/documents`): the complete, unpaginated document history, sorted most-recent-first (the backend's own `ORDER BY created_at DESC`, not re-sorted client-side). This is where full document management (including delete) happens: the full `ClinicalDocumentCard`/`ClinicalDocumentList` lives here, not on Overview. Three page-level actions sit in a `<nav aria-label="Document actions">`: "Upload Documents" (`patientUploadPath`), "Create Analysis" (`createAnalysisPath`, see Create Analysis above), and "View Patient" (`patientDetailPath`).

`src/components/documents/`:

- `ClinicalDocumentCard`: title, document type label (`documentTypeLabel`, in `api/clinicalDocuments.ts` so both this card and `NoteCard` share one lookup instead of two copies), creation date, a file-type label ("Pasted note" / "Uploaded .txt file" / "Uploaded .pdf file" / "Uploaded .csv file", read off `file_type`), and an analysis-count label ("Not yet analyzed" / "Used in `N` analyses") in the subtitle. The entire title/metadata block is itself a `<button>` (`aria-expanded`, `aria-controls`, `aria-label="{View/Hide} {title}"`) toggling an inline expand/collapse showing the document's `raw_text`, not a modal, and not a PDF preview: the backend only ever stores the extracted text (`raw_text`), never the original uploaded file bytes, so there is no file to preview or download, and no real byte size to show either (see Backend below). A fake "Download original PDF" action would be invented functionality; showing the real extracted text is the honest equivalent. "Delete" is a separate sibling button (nesting a button inside a button isn't valid), and mirrors `MedicationCard`'s pattern (own loading/error state, calls `onDelete`, parent hook updates local state on success).

  The whole title/metadata block is clickable to expand, matching `RecentDocumentCard`'s exact interaction on `PatientOverviewPage` (click anywhere on the text to expand) while keeping Delete as its own separate control. The two cards remain intentionally different in one respect: `RecentDocumentCard` has no Delete action, since that stays a `PatientDocumentsPage`-only capability.
- `ClinicalDocumentList`: a semantic `<ul>` of `ClinicalDocumentCard`s, used only by `PatientDocumentsPage`.
- `RecentDocumentCard` / `RecentDocumentsList`: the compact preview cards for Overview, mirroring `RecentAnalysisCard`/`RecentAnalysesList`'s "preview card, not the full one" relationship. The whole card is a `<button aria-expanded aria-controls>` (not a `<Link>`, since there is no separate document-detail route) toggling the same inline `raw_text` view `ClinicalDocumentCard` shows, with no Delete action; that stays on `PatientDocumentsPage`.
- `EmptyDocumentsState`: "No documents uploaded" plus a link to this patient's Upload page. Mirrors `AnalysesEmptyState`'s shape rather than `EmptyMedicationState`'s plain-text form, since (unlike Medications) there is no inline add-form directly below it to point at instead. Reused as-is by `PatientOverviewPage`'s preview, `PatientDocumentsPage`'s full view, and `CreateAnalysisPage`'s Existing Documents section (see Create Analysis above): the one place it's reused alongside a working alternative (the same page's own Upload Additional Documents section) rather than as a dead end.

#### Backend

`ClinicalDocument` has an `analyses` relationship (the same many-to-many `analysis_clinical_documents` table `Analysis.clinical_documents` uses), exposed over the API as an `analysis_count` property on the model (`len(self.analyses)`, the same pattern as `Analysis.document_count`) and a matching `analysis_count: int` field on `ClinicalDocumentResponse`. `get_clinical_documents_for_patient`/`get_clinical_document` use `selectinload(ClinicalDocument.analyses)` to avoid an N+1 query. See `docs/api.md`.

File size is **not** included, and can't be without a larger, unrelated change: the backend has never stored the original uploaded file's bytes, only the extracted `raw_text`. There is no byte count anywhere to expose honestly, so the UI simply omits it rather than substituting `len(raw_text)` or another number that isn't actually "file size."

#### Searching a patient's clinical documents

`PatientDocumentsPage` has a search box (`Input type="search" label="Search documents"`) above the list, shown only once there's at least one document to search. It's hidden entirely alongside `EmptyDocumentsState`, the same "nothing to search yet" reasoning `PatientsPage`/`DashboardPage` apply to `PatientSearch`. It reuses `filterClinicalDocuments` (`src/components/documents/filterClinicalDocuments.ts`) unchanged, the same pure, client-side, title-or-document-type-label matcher `CreateAnalysisPage`'s Existing Documents section uses (see Create Analysis above; the backend's `GET /patients/{patientId}/clinical-documents` has no search parameter, so this remains client-side filtering over the already-fetched list, not a request). A filtered-to-empty result shows "No documents match your search." in place of the list, distinct from `EmptyDocumentsState`'s "no documents at all" case, which stays reachable only when the *unfiltered* list is empty. Unlike `CreateAnalysisPage`'s compact Existing Documents section, there is no result cap or "Show more" here: this page is already the complete, unpaginated document history (see above), so search only ever narrows what's already fully visible, not what's hidden behind a preview limit.

### Quick Actions and the "Create Analysis" question

`PatientOverviewPage`'s Quick Actions bar (a `<nav aria-label="Quick actions">`) has three entries: **Create Analysis** (`createAnalysisPath`, primary/blue, the main value-generating action), **Upload document** (`patientUploadPath`, secondary/outline), and **Manage medications** (`patientMedicationsPath`).

This reflects a deliberate non-duplication principle: a Quick Actions entry only exists when it points somewhere genuinely distinct. Upload document (saving new files/notes to the patient's record) and Create Analysis (building and starting an analysis from existing documents, new uploads, or both, on `CreateAnalysisPage`, see Create Analysis above) are two different destinations doing two different things, so both earn a place in Quick Actions. A second button pointing at the same destination as an existing one would just be a duplicate, not a second real action, which is why Quick Actions doesn't list every possible entry point into every page; each entry here leads somewhere nothing else in this bar already goes.

### Archiving

`DELETE /patients/{patient_id}` is a soft delete (sets `status: "archived"`, never removes the row), and the UI's copy is deliberate about that: the confirmation dialog says the patient is "removed from your active patient list," never "deleted." Archiving is reachable from both `PatientsPage` (removes the card from view) and `PatientOverviewPage` (navigates back to `/patients` on success, since there's nothing left to show). Both pages own their own `patientPendingArchive`/`isArchiving`/`archiveError` state around the one shared `ArchivePatientDialog`, rather than that state living in a hook, since it's page-local UI state, not data the rest of the app needs.

No toast/notification component exists anywhere in this codebase, so success feedback is the immediate UI change itself: the card disappearing from the list, or the redirect back to a list that no longer contains the archived patient, the same feedback pattern every other remove/delete action in this app uses (`MedicationCard`, `NoteCard`, `UploadedFileList`). Archiving a patient never touches their medications, documents, or analyses; it only changes the patient's own `status`, and every one of those child resources stays fully reachable directly, exactly as before.
...
Medication management lives entirely within a patient's context, via `POST/GET/PATCH/DELETE /patients/{patient_id}/medications` on the backend (`docs/api.md`), and there is no route, page, or nav link that lists medications without a patient already selected.

Two places medications appear:

- `PatientMedicationsPage` (`/patients/:patientId/medications`): the full CRUD experience. Reads `:patientId` from the route, fetches the patient (`usePatient`, for the page title and a "← Back to {name}" link) and their medications (`usePatientMedications`), and renders the add form plus the full list.
- `PatientOverviewPage`'s Medications section: the same list (view/edit/delete inline, reusing `MedicationList`/`MedicationCard` directly, `usePatientMedications` called a second time with the same `patientId`) plus a "+ Add medication" link into `PatientMedicationsPage`. Deliberately does *not* embed `MedicationForm` a second time, since having two separate places a medication could be created would fragment the UI, so Overview only ever links out to the one page that owns the add form.

`usePatientMedications(patientId)` (`src/hooks/usePatientMedications.ts`) fetches on mount and re-fetches whenever `patientId` changes, so navigating from one patient's medications to another's never shows stale data. It exposes `addMedication`/`editMedication`/`removeMedication`, each threading `patientId` through to `api/medications.ts` and then updating local state directly from the response rather than refetching. These three functions don't catch their own errors; the calling component owns its own submitting/error UI. A fourth function, `importMedicationsCsv`, exists for CSV import; see below.

Every component in `src/components/medications/` (`MedicationForm`, `MedicationFields`, `MedicationCard`, `MedicationList`, `EmptyMedicationState`, `medicationFormValidation.ts`) never references `user_id` or ownership at all; threading `patientId` through the hook and API layer is enough on its own. `EmptyMedicationState` has an optional `message` prop (default preserves its original copy), so `PatientOverviewPage` shows "No medications recorded yet." instead of the default "...use the form below" copy, which would otherwise be inaccurate there (the add form lives on a different page in that context).

Validation (`medicationFormValidation.ts`) mirrors `schemas/medication.py` exactly: `medication_name`, `dose`, `route`, `frequency`, and `status` must all be non-empty; `notes` is optional. `source` is hardcoded to `"patient_reported"` in `api/medications.ts`, never user-facing.

### Searching a patient's medications

`PatientMedicationsPage` has a search box (`Input type="search" label="Search medications"`) above the list, shown only once there's at least one medication to search, hidden alongside `EmptyMedicationState`, the same pattern `PatientDocumentsPage`'s equivalent search box uses (see Clinical Documents above). `filterMedications` (`src/components/medications/filterMedications.ts`, alongside `filterPatients.ts`/`filterClinicalDocuments.ts`) is a pure, client-side matcher over medication name, dose, route, frequency, or status, case-insensitively, broader than `filterClinicalDocuments`'s two fields, since every one of those five is short text already shown on `MedicationCard` and each is a plausible thing a provider searches for (finding every medication with a given status, not just one by name). A filtered-to-empty result shows "No medications match your search." in place of the list, distinct from `EmptyMedicationState`'s "no medications at all" case. `PatientOverviewPage`'s own Medications section (unlike its capped Recent Clinical Documents preview, it already renders the patient's *full* medication list inline, see above) does not get this search box; it links out to `PatientMedicationsPage` via "+ Add medication" already, and that remains the one place medication search lives, consistent with `MedicationCsvUpload` and `MedicationForm` below, both of which are also `PatientMedicationsPage`-only.

### CSV import

Built directly against the patient-scoped `POST /patients/{patient_id}/medications/import` endpoint (`docs/api.md`). `MedicationCsvUpload` (`src/components/medications/MedicationCsvUpload.tsx`) is a dedicated, always-visible section on `PatientMedicationsPage`, between the medication list and the single-add `MedicationForm`, not a modal or collapsible, since the page already establishes "always-visible form below the list" as its interaction pattern (`MedicationForm`), and CSV import is just another way of adding medications. It is not shown on `PatientOverviewPage`.
...
Not to be confused with CSV support in Create Analysis (see "CSV medication list uploads" under Upload above): `MedicationCsvUpload` here imports rows directly into the medication list through its own dedicated drop target (deliberately not the shared `FileDropzone`), while `CreateAnalysisPage`/`UploadPage`'s `FileDropzone` accepting a `.csv` stores it as an ordinary analysis document instead. The two components, hooks, and API calls are entirely separate; nothing is shared beyond both accepting a `.csv` file.

**Import result handling**: the response (`MedicationImportSummary`: `rows_processed`, `medications_created`, `blank_rows_ignored`) has no created-medication objects to merge into local state, so `usePatientMedications`'s `importMedicationsCsv` triggers a refetch (bumping the same internal `retryCount` `retry()` uses) rather than trying to synthesize new rows locally. A successful import shows a `role="status"` message with the counts and clears the selected file; a failed one keeps the file selected (so the user doesn't have to reselect it to retry) and shows a `role="alert"`.

**File-level vs. row-level errors**: the CSV import endpoint is the only one in this app whose `422` `detail` is neither a plain string nor FastAPI's usual list of field errors, but a nested object (`{ message, row_errors }`). `toApiError` (`api/client.ts`) recognizes an object-shaped `detail` with a `message` field and preserves the *whole* raw `detail` on an optional `ApiError.detail`. `importMedicationsCsv` (`api/medications.ts`) checks `detail` for `row_errors` and, if present, attaches them as `rowErrors` on the thrown error, letting `MedicationCsvUpload` render a summary message plus a per-row list (row number preserved) distinctly from a plain file-level error, which has no `rowErrors` and renders as just the message.

Only one file at a time: the file input has no `multiple` attribute. No drag-and-drop: a plain `<input type="file">` is already fully keyboard-operable, and drag-and-drop is reserved for cases where it's genuinely simpler, which a single required file input isn't.
...
`ProfileSettings` (`src/components/settings/ProfileSettings.tsx`) edits First Name, Last Name, Username, and Email Address against the real backend.

The backend `User` model (`backend/app/models/user.py`) has only `email` and a single optional `name` string for a person's name, with no `first_name`/`last_name` columns. `profileFormValidation.ts`'s `splitName`/`joinName` bridge that gap entirely client-side: `splitName` takes the first whitespace-separated word as "First name" and everything after it as "Last name" (so "Ann Marie Mathew" round-trips as `firstName: "Ann"`, `lastName: "Marie Mathew"`, not silently dropping the middle name); `joinName` reverses it with a single space before saving. This is a deliberate, disclosed heuristic over one real column, not two real fields, and there is no plan to add real `first_name`/`last_name` columns, unlike Username below, which is a real, persisted backend field, not a client-side heuristic.

Saving calls `PATCH /users/me` (`updateUser`, `src/api/auth.ts`) and, on success, pushes the returned `User` into `AuthContext` via `setUser` (`AuthProvider.tsx`), the same context `TopNav`'s "Welcome back, {name}" and the Dashboard's `PageHeader` already read, so a name change is reflected everywhere immediately, with no second `GET /users/me` and no page reload. A changed email or username that collides with another account surfaces the backend's `409` attached to the matching field specifically (mirroring `SignupPage`'s existing 409-to-field pattern, distinguished by checking whether the backend's message text mentions "username," since both failures share the same status code); every other failure is a generic form-level `FormError`. Validation (`validateProfileForm`) mirrors the app's established per-form pattern (`patientFormValidation.ts`, `SignupPage`'s inline `validate`): First Name, Last Name, and Email are required (Email must additionally pass `isValidEmail`, `src/utils/validation.ts`); Username is the one field that's optional here (see below).

**Username.** Real, persisted, and enforced backend-side: 3-30 characters, letters/numbers/underscores/periods only, unique case-insensitively (`docs/api.md`, `docs/data-model.md`). Two things make this field different from every other one on this form:

- **It's the only field that validates live, on every keystroke, rather than only on submit** (`handleUsernameChange` calling `usernameFormatError`, `src/utils/validation.ts`, the same format check `SignupPage`'s own username field uses, so the two forms can never enforce different rules). A username has more distinct ways to be malformed (length, character set) than this form's other fields, and format mistakes are cheap to flag immediately rather than waiting for a round trip.
- **It's optional here, unlike on `SignupPage`.** An existing account may never have set one (`user.username` is `null`), and leaving the field blank means "no change" (unset) rather than a validation error, so `validateProfileForm` skips its format check entirely for an empty value. Submitting sends `username: null` for a blank field, never `''`, since the backend's own minimum-length rule would otherwise reject an empty string as too short for a field that's supposed to mean "not set."

### Where a username is shown instead of an email

The only place in the app that shows a resolving/owning user's email as a fallback is `MedicationDiscrepancyCard`'s "Resolved by {who}" line (see Analyses above), which falls back from `name` to `email` when a resolver has no name on file. It checks `username` first: `resolved_by.username ?? resolved_by.name ?? resolved_by.email`. `TopNav` and the Dashboard's "Welcome back, {name}" greeting deliberately don't do this: both already read `name`, not `email`, so there's no email-as-identity display there to replace.

### Appearance

`AppearanceSettings` (`src/components/settings/AppearanceSettings.tsx`) is a two-step picker over `useTheme()` (see Theming below): a **Palette** grid (9 options, `PALETTE_OPTIONS`, `ThemeContext.ts`) and, independently, a **Mode** toggle (Light / Dark / High Contrast / System, `MODE_OPTIONS`). The two are genuinely independent state (`palette`, `modePreference`): picking a different palette never changes the current mode and vice versa, so "Twilight, but in Dark" and "Twilight, but High Contrast" are just two clicks from any starting point, not two separate named themes to hunt for in one long list.
...
`src/components/common/`: `Button`, `Input`, `Card`, `PageHeader`, `LoadingSpinner`, `ErrorState`, `SummaryStat`, `ProtectedRoute`, `PublicOnlyRoute`, `BackButton` (see Patient breadcrumb navigation above).
...
The interface is designed to work well at mobile/tablet/desktop widths without a separate layout system: responsive behavior is built from a small set of reusable Tailwind utility patterns applied consistently app-wide (see Dashboard's two-column `grid grid-cols-1 lg:grid-cols-2` and `PatientCard`'s `flex flex-col sm:flex-row` card for two examples), rather than each page inventing its own approach.
...
- **Dialogs get an explicit gutter on narrow screens.** `ArchivePatientDialog`, `DeleteAnalysisDialog`, and `StartAnalysisDialog` are native `<dialog>` elements centered via `fixed inset-0 m-auto h-fit` (see Accessibility below) with a `max-w-sm`/`max-w-md` cap and `w-[calc(100%-2rem)]`, which guarantees a 1rem gutter on every side no matter how narrow the viewport is, while the `max-w-*` cap still applies unchanged on larger screens. Plain `w-full` would resolve to 100% of the viewport whenever the viewport is narrower than the cap (any phone ≤384px/448px wide); with `m-auto` centering having no leftover space to distribute, the dialog would touch both edges of the screen, which is what `w-[calc(100%-2rem)]` avoids.
- **Every raw form control fills its container.** `Input`'s base `<input>` and `DocumentTypeSelect`'s `<select>` have `w-full`, and the same is applied directly to every raw `<textarea>` in the app (`PatientFields`, `MedicationFields`, `ManualNoteEditor`, `NoteCard`'s edit mode); without it, a form control renders at the browser's intrinsic control width (effectively fixed, ~170-200px) regardless of its container, unless a page happens to wrap it in something that forces a width. Fixing the shared base components fixes every form at once, with no page-level changes needed.

No page is redesigned to achieve this and no new components are introduced; every pattern above is a className addition (wrapping div/span elements already exist; nothing new is added to the DOM tree except where noted). `AnalysisProcessingPage`'s 2-stat `grid-cols-2` is left without a breakpoint variant (unlike `AnalysisDetailPage`'s 3/4-stat grids, which scale via `sm:grid-cols-3`/`sm:grid-cols-4`); two short stats side by side don't need one to stay usable at any width.

---

## Accessibility

These are the baseline accessibility practices the app follows, not a claim of full WCAG compliance:
...
- Every native-`<dialog>` component (`ArchivePatientDialog`, `DeleteAnalysisDialog`, `StartAnalysisDialog`) explicitly centers itself with `fixed inset-0 m-auto h-fit` on the `<dialog>` element itself, rather than relying on the browser's own centering. Browsers normally center a modal `<dialog>` via a UA-stylesheet `margin: auto`, but Tailwind's Preflight reset (`@import 'tailwindcss'` in `globals.css`) zeroes `margin` on every element including `<dialog>`, silently overriding that default; without this explicit centering, the dialog would render pinned to the viewport's top-left corner instead of centered.
- `BackButton` is a real `<button>`, not a styled `<span>` or `<div>`, so it's keyboard-focusable and activatable with Enter/Space by default. Its decorative "←" is wrapped in its own `aria-hidden="true"` span so the button's accessible name is exactly "Back to {label}," not "left arrow Back to {label}." `PatientBreadcrumb`'s `<nav aria-label="Breadcrumb">` plus `aria-current="page"` on the final crumb together satisfy "breadcrumbs use appropriate semantic navigation" and "current page is identified correctly."
- `ResolveDiscrepancyDialog` (see Analyses above) follows the same native-`<dialog>`/explicit-Cancel-focus pattern as `DeleteAnalysisDialog`, and is the confirmation step for every discrepancy resolution action, including the one with no undo (`Dismiss`), not just a subset of them. Each resolution action button has a descriptive `aria-label` naming both the action and the medication it applies to, rather than a bare "Dismiss"/"Edit Manually" repeated identically across every card on the page. The post-resolution success message uses `role="status"`, matching `LoadingSpinner`'s existing convention for announcing a non-error state change.

---

## State Management

Global state uses React Context (`AuthContext` today). No Redux or other state library is introduced; the app's state isn't yet complex enough to justify one.
...
### Quality pipeline: linting, type checking, formatting
...
**`docker build` is the workflow's final step.** After `npm run build` passes, the job runs `docker build --build-arg VITE_API_BASE_URL=${{ env.VITE_API_BASE_URL }} -t medlens-frontend:ci .` from `frontend/`, proving the production image (`frontend/Dockerfile`) still builds on every push and PR. This is deliberately the last step of the *same* `quality` job, not a second job or workflow file, matching `backend.yml`'s identically-placed step (see `docs/testing.md`). `VITE_API_BASE_URL` is passed as a build arg (reusing the same value the job's `env:` block already sets) because Vite inlines it into the built JS bundle at build time, so it has to reach the build through `--build-arg`, not just be present in the process environment, unlike every step above it. See `docs/deployment.md`'s Docker Image Builds section for the Dockerfile itself (multi-stage, nginx, non-root-friendly) and `docs/design-decisions.md` (Decision 18) for why.

**ESLint** (`eslint.config.js`, flat config) extends `@eslint/js`'s recommended rules, `typescript-eslint`'s recommended rules, `eslint-plugin-react-hooks`'s recommended rules, and `eslint-plugin-react-refresh` (Vite's Fast Refresh boundary rule), then layers `eslint-config-prettier` last so Prettier owns 100% of formatting: ESLint never argues with Prettier over a stylistic call, and there are no formatting-motivated `eslint-disable` comments anywhere in the codebase. There's no separate "Vite" ESLint plugin: `eslint-plugin-react-refresh`'s `only-export-components` rule *is* the Vite-specific check (it catches files that break Fast Refresh by exporting something other than components). No `eslint-plugin-react` (the older, pre-React-17 plugin providing rules like `react/jsx-uses-react`) is installed or needed, since this app uses the automatic JSX runtime (`"jsx": "react-jsx"` in `tsconfig.app.json`), which is exactly the case that plugin's own docs say to skip it for.

**TypeScript** is configured strictly: `strict`, `noUncheckedIndexedAccess`, `noImplicitOverride`, `noUnusedLocals`, `noUnusedParameters`, and `noFallthroughCasesInSwitch` in `tsconfig.app.json`, mirrored (minus the DOM-specific settings) in `tsconfig.node.json` for `vite.config.ts`. `npm run typecheck` runs the same project-references build (`tsc -b`) that's also half of `npm run build`, made runnable on its own. Both sub-configs set `noEmit: true`, so this genuinely only type-checks; it writes no `.js` output (it does still write its `.tsbuildinfo` incremental-build cache to `node_modules/.tmp/`, same as `build` does).

**Prettier** (`.prettierrc.json`: no semicolons, single quotes, 100-char print width, trailing commas) is wired into ESLint via `eslint-config-prettier`; every source file stays in agreement on style since `eslint-config-prettier` and Prettier have no material rules to add against each other.

No dependencies beyond what `lint`/`typecheck`/`format` need are required; each is already a `devDependency`.
...
**Local development runs the frontend via `npm run dev`, not Docker** (see the root `README.md`): the `frontend` service in `infra/docker-compose.yml` builds and serves the *production* image (`frontend/Dockerfile`: a Vite build served by nginx, no hot reload), which exists to validate that image builds and runs, not to replace the dev server's fast local iteration loop. Both can run at once without colliding: the dev server's default port (5173/5174) is unrelated to the container's host-mapped port (`8080`, see `infra/docker-compose.yml`). See `docs/deployment.md`'s Docker Image Builds section for the Dockerfile itself.
...
- A "Key clinical observations", "Mentioned conditions", or "Notable findings" section on `AnalysisDetailPage`'s AI Summary: the AI response schema (`app/ai/schemas.py`) has no structured field for any of these; see Analyses above.
- A "Follow-up Questions" checklist rendering `possible_inconsistencies`. Such a section existed at one point and was removed, since the heading didn't match the content (statements, not questions) and its checkbox had no real state behind it. The backend field itself is untouched; only the frontend rendering was removed. See Analyses above.
- Access-timestamp tracking for a truer "recently accessed" patient ordering on the Dashboard: `sortPatientsByRecentActivity` uses `updated_at`/`created_at` instead (see Dashboard above), since there is no "last opened by this provider" timestamp anywhere in this app. This is unrelated to the Dashboard's separate Recent *Analyses* feed, which does exist (see Dashboard above).
- Provider-level analytics or notifications (none requested, none built). Settings exists (see Settings and Theming above).
- A real backend `first_name`/`last_name` column split. `ProfileSettings`' First/Last Name split is a client-side heuristic over the single `name` column; see Settings above. (Username, unlike name, is a real, persisted backend field; see Settings above.)
