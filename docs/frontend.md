...
- **Grouping and sorting** - `groupBySeverity` (`AnalysisDetailPage.tsx`) buckets findings into high/medium/low groups, in that order, dropping empty groups; within a group, findings are sorted by ascending `id` (stable, matching the order the backend already returns them in). Each non-empty group renders under its own `<h3>` ("High severity (2)"). There is no "critical" tier: `DiscrepancySeverity` (the backend enum) only has `high`/`medium`/`low` - the frontend reuses the existing enum rather than inventing a 4th tier that would require a backend change.
...
- `useAnalysisDetail`'s `replaceDiscrepancy(updated)` splices the returned, fully-resolved `MedicationDiscrepancy` into `analysis.medication_discrepancies` in place (matched by `id`), so the card immediately re-renders with its resolved state - no full analysis refetch, and no risk of a second discrepancy's own concurrent edit being clobbered by a stale refetch response.
...
Medication management lives entirely within a patient's context - `POST/GET/PATCH/DELETE /patients/{patient_id}/medications` on the backend (`docs/api.md`) - and there is no route, page, or nav link that lists medications without a patient already selected.
...
- **It's optional here, unlike on `SignupPage`.** An existing account may never have set one (`user.username` is `null`), and leaving the field blank means "no change" (unset) rather than a validation error - `validateProfileForm` skips its format check entirely for an empty value. Submitting sends `username: null` for a blank field, never `''`, since the backend's own minimum-length rule would otherwise reject an empty string as too short for a field that's supposed to mean "not set."
