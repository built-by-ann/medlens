...
| `/patients/:patientId/analyses/select-documents` | `SelectDocumentsPage` | `AppLayout` | yes |
...
Issue #145 added a second way into analysis creation, alongside Upload: `SelectDocumentsPage` (`src/pages/SelectDocumentsPage.tsx`, `/patients/:patientId/analyses/select-documents`) lets a provider pick from a patient's *already-uploaded* clinical documents instead of supplying new ones. Backend investigation for this issue found `POST /patients/{patientId}/analyses` never distinguished a newly uploaded document from one already on file - it only ever validates that every requested id belongs to the given patient (`create_analysis`, `app/services/analysis_service.py`) - so **no backend change was needed**; see `docs/api.md`.

The page fetches the patient (`usePatient`) and the patient's documents (`usePatientClinicalDocuments(patientId)`, the same hook the Clinical Documents section on `PatientOverviewPage` uses), and renders each as a `<label>` wrapping a real `<input type="checkbox">` plus the document's title, `documentTypeLabel(document_type)`, and upload date - the label wrapping means clicking anywhere in the row toggles it, and gives each checkbox a correct accessible name (the title, from the label's own text content) for free, with no separate `aria-label` needed. Selected ids live in local `Set<number>` state; a "Create Analysis" button is disabled until at least one is selected, next to an `aria-live="polite"` "`N` of `M` documents selected" count so both sighted and screen-reader users can tell selection changed. When the patient has no documents at all, the page reuses `EmptyDocumentsState` (`components/documents/EmptyDocumentsState.tsx`) as-is rather than a near-duplicate - same "upload your first document" copy and CTA already used on `PatientOverviewPage`.

Submitting does **not** call `useCreateAnalysis` itself here - it just navigates to `analysisProcessingPath(patientId)` with `{ files: [], notes: [], existingDocumentIds }` as router state, exactly the shape `UploadPage` already navigates with (empty `files`/`notes`), so `AnalysisProcessingPage` needs no changes at all to handle this second entry point - see below. `useCreateAnalysis`'s `SubmitInput` gained one new optional field, `existingDocumentIds?: number[]` (`src/hooks/useCreateAnalysis.ts`): `submit()` seeds `clinicalDocumentIds` with them before the existing upload-files/create-notes loops run, so a submission can mix newly uploaded items with reused ones in a single `createAnalysisFromDocuments` call, though today only `UploadPage` (all new) and `SelectDocumentsPage` (all existing) actually produce one or the other, never both.

The entry point itself lives on `PatientOverviewPage`'s Clinical Documents section header, next to the "Clinical documents" heading: a "Create analysis from documents" link to `selectDocumentsPath(patientId)`, shown unconditionally (a patient with zero documents still reaches the page, which handles that with its own empty state above) - see "Quick Actions and the 'Create Analysis' question" under Patients, which this issue's addition directly follows up on.

---

## Analysis Processing

Issue #44 added a dedicated page bridging analysis creation and `AnalysisDetailPage`: `AnalysisProcessingPage` (`src/pages/AnalysisProcessingPage.tsx`, `/patients/:patientId/analyses/processing`). As of Issue #145 it has two entry points - `UploadPage`'s "Start Analysis" and `SelectDocumentsPage`'s "Create Analysis" - both of which simply `navigate()` here with a `SubmitInput`-shaped router state; this page has no idea which one sent it, and needed no changes to support the second. It reads that `files`/`notes`/`existingDocumentIds` from `useLocation().state` rather than the URL, since a File object can't round-trip through a path or query string; visiting the route directly or reloading it loses that in-memory state, so the page detects a missing/empty `state` and shows a "Nothing to process" recovery card linking back to Upload instead of hanging on a spinner forever.
...
- `EmptyDocumentsState`: "No documents uploaded" plus a link to this patient's Upload page, shown when the list is empty - mirrors `AnalysesEmptyState`'s shape rather than `EmptyMedicationState`'s plain-text form, since (unlike Medications) there is no inline add-form directly below it to point at instead. Reused as-is by `SelectDocumentsPage`'s own empty state (Issue #145 - see Select Existing Documents above).

As of Issue #145, the Clinical Documents section header also carries a "Create analysis from documents" link to `selectDocumentsPath(patientId)`, next to the "Clinical documents" heading - see Select Existing Documents above.
...
**Issue #145 is the issue that closed that gap.** A genuinely distinct destination now exists - `SelectDocumentsPage` - so its entry point was added where it actually belongs: the Clinical Documents section header, not Quick Actions (see Clinical Documents above and Select Existing Documents above). Quick Actions itself is unchanged; "Create Analysis" as a concept was and remains reachable through both `UploadPage` ("Start Analysis") and, now, `SelectDocumentsPage` ("Create Analysis"), and `PatientAnalysesPage` still offers its own "+ Start analysis" link into the upload flow for anyone arriving from the analysis history instead.
