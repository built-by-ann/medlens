...
3. **Patient search** - `PatientSearch` (unchanged) feeds the existing `filterPatients` utility (`components/patients/filterPatients.ts`, from Issue #127 - client-side, case-insensitive, over first name/last name/full name/MRN, since the backend's `GET /patients` has no search parameter and this issue does not add one). Search runs live on every keystroke (no debounce needed - `filterPatients` is a synchronous, cheap array filter) and searches the *entire* loaded patient list, not just the Recent Patients preview below, so a patient outside the top 5 is still found.
...
`UploadPage` (`src/pages/UploadPage.tsx`) is nested under a patient: `/patients/:patientId/upload`. It fetches the patient via `usePatient` (the same hook `PatientOverviewPage` and `PatientMedicationsPage` use) to show the patient's name in the heading, a breadcrumb, and a back link, then lets the user supply one or more clinical notes for that patient, either as files or pasted text. This is the same workflow introduced in Issue #41, adapted (not rebuilt) first for Sprint 3.5 (Issue #130 - patient-scoping), again for Issue #131 (the breadcrumb/back-link chrome), and again for Issue #44 (see Analysis Processing below): as of Issue #44, `UploadPage` itself no longer submits anything - clicking "Start Analysis" (after client-side validation that at least one file or note was added) navigates straight to `analysisProcessingPath(patientId)`, passing the queued `files`/`notes` as router `state`. The actual upload-and-create sequence - `POST /patients/{patientId}/clinical-documents` (pasted text), `/patients/{patientId}/clinical-documents/upload-txt`, `/patients/{patientId}/clinical-documents/upload-pdf` (files), and `POST /patients/{patientId}/analyses` (create the analysis) - now runs from the new Analysis Processing page instead, via the same `useCreateAnalysis(patientId)` hook (`src/hooks/useCreateAnalysis.ts`). Every document created through this flow belongs to the selected patient only - there is no global document pool to select from or accidentally mix into another patient's analysis.
...
`AnalysisDetailPage` (`src/pages/AnalysisDetailPage.tsx`, `/patients/:patientId/analyses/:analysisId`) fetches real data via `useAnalysisDetail(patientId, analysisId)` (`src/hooks/useAnalysisDetail.ts`) calling `getAnalysisDetail` (`api/analyses.ts`, `GET /patients/{patientId}/analyses/{analysisId}`). Issue #131 first wired this page up with real metadata only, deliberately leaving the AI's raw findings and any discrepancy display unbuilt. **Issue #45 is that future issue**: the frontend `AnalysisDetail` type (`types/api.ts`) no longer omits `medication_mentions`, `possible_inconsistencies`, or `medication_discrepancies` - it mirrors the backend's `AnalysisDetailResponse` in full - and the page is now the primary destination for a completed analysis, organized top to bottom as:

1. **Breadcrumb** (Patients / patient name / Analyses / Analysis #N, unchanged) plus an explicit `<nav aria-label="Analysis navigation">` with three links every completed analysis needs a way back through: Patient Overview, Analysis History, and Clinical Documents (`${patientDetailPath}#documents-heading`, jumping straight to that section on Overview).
2. **Page header** - title is now the generic "Analysis Results" (the page's actual identity), with `Analysis #{id} for {patient name}` moved to the description and the `AnalysisStatusBadge` still in `actions` for completion status at a glance. The sanitized `error_message` alert for a failed run is unchanged from Issue #131.
3. **Patient Information** - reuses `PatientDetails` (`components/patients/PatientDetails.tsx`, the same card `PatientOverviewPage` renders) directly rather than duplicating patient fields a second way.
4. **Analysis Summary** - a `Card` with the AI `summary` text (or "No AI summary was generated for this analysis." when `null`) plus three `SummaryStat`s computed entirely from data already on the response: discrepancies found (`medication_discrepancies.length`), medications analyzed (`medication_mentions.length`), documents analyzed (`document_count`, new in Issue #45 - see Backend below).
5. **Medication Reconciliation Findings** - one `MedicationDiscrepancyCard` (`components/analyses/MedicationDiscrepancyCard.tsx`) per `medication_discrepancies` row, sorted highest-severity-first (`utils/discrepancy.ts`'s `DISCREPANCY_SEVERITY_RANK`, tie-broken by id). Each card shows severity (`DiscrepancySeverityBadge` - a colored badge plus a text label like "High severity", and a matching left border repeating the same signal, so severity is never color-only), the medication name, discrepancy type (`discrepancyTypeLabel`), resolution status (`ResolutionStatusBadge`), the `ai_explanation` description, `expected_value`/`observed_value` when present, and a "Supporting evidence" subsection nested inside the same card (not a separate page section) so evidence always stays visually attached to the finding it supports: the source document's title/type and the mention's `context_text` snippet when `medication_mention` is present, a one-line summary of the current medication list entry when only `medication` is present, or an explicit "No supporting evidence was recorded for this finding." when neither is. An empty findings list itself renders "No medication reconciliation findings are available for this analysis." rather than nothing - deliberately neutral wording, not "no problems found," since (see Backend below) an empty list today most often means reconciliation never ran, not that it ran and found nothing.
6. **Analysis Metadata** - analysis ID, created/completed timestamps, provider, model, and a client-computed processing duration (`completed_at - started_at`, formatted as `Ns` or `Nm Ns`, omitted when either timestamp is missing - no backend change needed for this one, both timestamps were already exposed).

#### Backend changes for Issue #45

`AnalysisDetailResponse` did not previously expose `medication_discrepancies` or `document_count` at all - reviewing the reconciliation engine (`app/services/medication_reconciliation_service.py`) surfaced that `run_medication_reconciliation`, the function that actually produces `MedicationDiscrepancy` rows, is never called by any route; `POST /patients/{patient_id}/analyses` only ever runs the separate AI-summary path. So today, `medication_discrepancies` is an empty list for every analysis created through the app's real upload workflow - wiring the reconciliation engine into analysis creation is a distinct, larger change than a results *page*, and stayed out of scope here. The smallest change that still lets this page render real findings once that wiring exists elsewhere: `MedicationDiscrepancyDetailResponse` (`schemas/medication_discrepancy.py`) extends the existing `MedicationDiscrepancyResponse` with nested `medication` (reusing `MedicationResponse` as-is) and `medication_mention` (a new, purpose-built `MedicationMentionEvidenceResponse`, since `MedicationMention` had no API exposure anywhere else - nesting a new minimal `ClinicalDocumentSummaryResponse` for its source document rather than the full `ClinicalDocumentResponse`, so a citation doesn't drag along the entire `raw_text`). `document_count` was added as a `document_count` property directly on the `Analysis` model (`len(self.clinical_documents)`) rather than only recomputed inline in the route, since `AnalysisSummaryResponse` already needed the identical value and `get_analysis_for_patient` needed to `selectinload` the same relationship anyway for the new discrepancy evidence. See `docs/api.md` for the full response shape and `docs/data-model.md` for the underlying models.
...
Sprint 3.5, Issue #131's main addition: a real document management section on `PatientOverviewPage`, replacing the placeholder line that used to sit there. `usePatientClinicalDocuments(patientId)` (`src/hooks/usePatientClinicalDocuments.ts`) fetches the patient's documents via `listClinicalDocuments` (`api/clinicalDocuments.ts`, `GET /patients/{patientId}/clinical-documents` - this call existed on the backend since Issue #130 but nothing on the frontend used it until now) and exposes `{ documents, isLoading, error, retry, removeDocument }`, the same shape as the other patient-scoped hooks; `removeDocument` calls the also-new `deleteClinicalDocument` (`DELETE /patients/{patientId}/clinical-documents/{documentId}`).

`src/components/documents/`:

- `ClinicalDocumentCard`: title, document type label (`documentTypeLabel`, moved from a private helper inside `NoteCard` into `api/clinicalDocuments.ts` so both consumers share one lookup instead of two copies), creation date, and a file-type label ("Pasted note" / "Uploaded .txt file" / "Uploaded .pdf file", read off `file_type`) in the subtitle. "View" toggles an inline expand/collapse (`aria-expanded`, `aria-controls`) showing the document's `raw_text` - not a modal, and not a PDF preview: the backend only ever stores the extracted text (`raw_text`), never the original uploaded file bytes, so there is no file to preview or download. Building a fake "Download original PDF" action would be exactly the invented functionality the issue warns against; showing the real extracted text is the honest equivalent. "Delete" mirrors `MedicationCard`'s pattern (own loading/error state, calls `onDelete`, parent hook updates local state on success).
- `ClinicalDocumentList`: a semantic `<ul>` of cards.
- `EmptyDocumentsState`: "No documents uploaded" plus a link to this patient's Upload page, shown when the list is empty - mirrors `AnalysesEmptyState`'s shape rather than `EmptyMedicationState`'s plain-text form, since (unlike Medications) there is no inline add-form directly below it to point at instead.

### Quick Actions and the "Create Analysis" question

`PatientOverviewPage`'s Quick Actions bar (a `<nav aria-label="Quick actions">`) has two entries: "Upload document" (`patientUploadPath`) and "Manage medications" (`patientMedicationsPath`). The issue's own recommended layout listed a third, separate "Create Analysis" action - deliberately not added, because it would have had nowhere distinct to go: creating an analysis only happens by selecting documents on `UploadPage` (see Upload above), the same destination "Upload document" already links to. A second button with a different label pointing at the identical route is exactly the "duplicate button" the issue warns against, not two real actions. "Create Analysis" as a concept is still fully reachable - once on `UploadPage`, adding at least one file or note and clicking "Start Analysis" *is* creating an analysis - and `PatientAnalysesPage` separately offers its own "+ Start analysis" link into the same flow for anyone arriving from the analysis history instead. Building a second, independent "create an analysis from documents already on file" flow (which would need its own document-selection UI) was treated as new functionality beyond this issue's "connect what exists" scope, not a gap this issue asked to close.
...
- Un-archiving.
- Selecting from a patient's already-uploaded documents to start a new analysis without re-uploading anything (see "Quick Actions and the 'Create Analysis' question" above) - today, an analysis can only be created from documents added in the same Upload session.
...
- Wiring the reconciliation engine into analysis creation, so `medication_discrepancies` is populated for analyses created through the app's real upload workflow rather than only where something calls `run_medication_reconciliation` directly (Issue #45 built the Analysis Results page that renders these findings and exposed them over the API; it deliberately did not change how or when an analysis is created - see Analysis Results above and `docs/api.md`).
- A Docker Compose service for the frontend.
- Selecting from a patient's already-uploaded documents to start a new analysis without re-uploading them (see "Quick Actions and the 'Create Analysis' question" under Patients above).
