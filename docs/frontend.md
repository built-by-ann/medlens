...
- ~~**Follow-up Questions**~~ - originally an `<h3>` rendering `possible_inconsistencies` as a checklist (`FollowUpQuestionsChecklist`); **removed entirely** - see below.
...
### Resolving discrepancies from the Analysis Results page (Complete Medication Reconciliation Workflow)

Everything up to this point rendered a discrepancy for review only - `MedicationDiscrepancyCard` (Issue #46, above) had no way to act on a finding. This work extends that same card and page rather than introducing a second results view, per the issue's own "extend the existing... architecture, don't redesign the layout" instruction.
...
This is the mechanism behind the issue's "do not expose actions that do not make sense for that discrepancy" requirement - a `MedicationDiscrepancyCard` never special-cases a discrepancy type in its own render logic to decide which buttons to show; it always renders exactly what `getDiscrepancyActions` returns, so adding or changing an action mapping never touches the card component. `Update Medication`'s single-field variant is additionally omitted when the mention never actually reported that field (e.g. a `dose_conflict` where `medication_mention.dose` is `null`) - there is nothing to one-click accept, so only Edit Manually and Dismiss remain. `requiredFieldKeys`/`buildResolutionPayload` (same file) complete the mapping: which of an action's fields must be non-empty before submission, and how to turn the dialog's current field values plus an optional note into the exact `DiscrepancyResolutionPayload` body `POST .../resolve` expects (trimmed, empty fields omitted, per `docs/api.md`'s request shape).

**`ResolveDiscrepancyDialog`** (`src/components/analyses/ResolveDiscrepancyDialog.tsx`) is the one dialog every action opens, editable or not - built on the same native `<dialog>` pattern `DeleteAnalysisDialog`/`ArchivePatientDialog`/`StartAnalysisDialog` already established (`showModal()`/`close()`, backdrop-click-to-cancel, a persistent `<dialog>` with `{target && (...)}` conditional content rather than two different dialog shapes). Its `target: ResolveDiscrepancyTarget | null` prop carries the chosen `DiscrepancyActionDescriptor` directly, so the dialog needs no discrepancy-type-specific branching either: when `action.editable` is true, it renders one labeled text input per `action.fields` entry (marked `*` and required when `requiredFieldKeys` includes it), pre-filled from the descriptor's own suggested values; when false (a one-click accept like "Update Medication" for a single-field conflict, or "Mark Discontinued"/"Mark Active"), the fields are hidden entirely and the dialog is just a confirmation with its `action.description` explaining what will happen. Every action's dialog also offers an optional "Note" textarea, submitted as `resolution_note` regardless of action - satisfying the issue's "optional provider notes" requirement uniformly rather than only for some actions. `Dismiss` opens the same dialog with no fields, satisfying "confirmation dialogs where destructive" for the one action here with no undo (see Accessibility below).

**Wiring, on `AnalysisDetailPage`**: `MedicationDiscrepancyCard` gained an optional `onResolveAction` prop; when provided and the discrepancy is still `open`, it renders the action buttons described above, each calling back with `{ discrepancyId, medicationName, action }`. `AnalysisDetailPage` uses that callback to open `ResolveDiscrepancyDialog`, and on confirm calls the new `resolveDiscrepancy(patientId, analysisId, discrepancyId, payload)` (`src/api/analyses.ts`, `POST .../resolve`). On success:

- `useAnalysisDetail`'s new `replaceDiscrepancy(updated)` splices the returned, fully-resolved `MedicationDiscrepancy` into `analysis.medication_discrepancies` in place (matched by `id`), so the card immediately re-renders with its resolved state - no full analysis refetch, and no risk of a second discrepancy's own concurrent edit being clobbered by a stale refetch response.
- A `role="status"` success message ("Lisinopril added to the medication list." / "Discrepancy dismissed.") is shown briefly, satisfying the issue's "status updates should be announced to assistive technology" and "provide immediate success feedback" requirements together with the one mechanism.
- Because `MedicationDiscrepancyCard` derives its own severity summary counts and grouping from the same `medication_discrepancies` array `AnalysisDetailPage` already holds (Issue #46), the Findings summary (Total/High/Medium/Low) and severity grouping stay correct automatically - resolving a finding never removes it from the array (dismissed and resolved findings both remain visible, per the issue's "dismissed discrepancies remain visible but clearly marked" requirement), only its own `resolution_status`/audit fields change.
- `RecentAnalysisCard`'s "Open" stat (backed by the new `open_findings` field - see `docs/api.md` and `docs/architecture.md`) reflects the change the next time that card's own data is fetched (Patient Overview's Recent Analyses preview, the Dashboard's cross-patient feed) - both already re-fetch on mount, so no additional cache-invalidation code was needed; this app has no client-side cache to invalidate (see State Management below).

**Resolved-state rendering.** Once `resolution_status` is no longer `open`, `MedicationDiscrepancyCard` stops rendering any action button for that card at all (editing a resolved finding is not possible - the issue's "once resolved, disable further editing" requirement - and there is no separate `disabled` styling to design, since the buttons are simply absent) and instead renders an audit block: "Resolved by {name or email} on {formatted resolved_at}" or "Dismissed by ..." (label depends on `resolution_status`), plus the `resolution_note` when present. `ResolutionStatusBadge` (Issue #46, unchanged) already visually distinguishes Open/Resolved/Dismissed with both color and a text label - never color alone - so a resolved or dismissed card is clearly marked at a glance in addition to the detailed audit line below it.

**Accessibility**, matching the issue's explicit requirements and this codebase's established patterns (see Accessibility below):
...
- `Dismiss` (the one action with no undo) opens the same confirmation dialog every other action does, rather than acting immediately on click - satisfying "confirmation dialogs where destructive" without a second, dismiss-only dialog component.
- On open, `ResolveDiscrepancyDialog` focuses its Cancel button first (the same explicit `cancelButtonRef.current?.focus()` pattern `DeleteAnalysisDialog` uses, since this project's jsdom `<dialog>` polyfill doesn't replicate the browser's own auto-focus-first-control behavior - see Issue #48 above), landing keyboard/screen-reader focus on the safest action by default.
...
`PatientDocumentsPage` gained a search box (`Input type="search" label="Search documents"`) above the list, shown only once there's at least one document to search - it's hidden entirely alongside `EmptyDocumentsState`, the same "nothing to search yet" reasoning `PatientsPage`/`DashboardPage` already apply to `PatientSearch`. It reuses `filterClinicalDocuments` (`src/components/documents/filterClinicalDocuments.ts`) unchanged - the same pure, client-side, title-or-document-type-label matcher `CreateAnalysisPage`'s Existing Documents section already established (see Create Analysis above; the backend's `GET /patients/{patientId}/clinical-documents` still has no search parameter, so this remains client-side filtering over the already-fetched list, not a request). A filtered-to-empty result shows "No documents match your search." in place of the list - distinct from `EmptyDocumentsState`'s "no documents at all" case, which stays reachable only when the *unfiltered* list is empty. Unlike `CreateAnalysisPage`'s compact Existing Documents section, there is no result cap or "Show more" here: this page is already the complete, unpaginated document history (see above), so search only ever narrows what's already fully visible, not what's hidden behind a preview limit.
...
**Username (Issue #191).** Real, persisted, and enforced backend-side - 3-30 characters, letters/numbers/underscores/periods only, unique case-insensitively (`docs/api.md`, `docs/data-model.md`). Two things make this field different from every other one on this form:
...
### Where a username is shown instead of an email (Issue #191)

Requires a place that already showed a resolving/owning user's email as a fallback in the first place - the only one in the app is `MedicationDiscrepancyCard`'s "Resolved by {who}" line (see Analyses above), which already fell back from `name` to `email` when a resolver had no name on file. It now checks `username` first: `resolved_by.username ?? resolved_by.name ?? resolved_by.email`. `TopNav` and the Dashboard's "Welcome back, {name}" greeting were deliberately left alone - both already read `name`, not `email`, so there was no email-as-identity display there to replace.
...
- Rendering `possible_inconsistencies` at all - Issue #47 added a "Follow-up Questions" checklist for it, later removed entirely (see Analyses above) since the heading didn't match the content (statements, not questions) and its checkbox had no real state behind it. The backend field itself is untouched; only the frontend rendering was removed.
