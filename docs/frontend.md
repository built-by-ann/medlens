...
`TopNav` links to Dashboard and Patients, and shows either a "Log in" link or a "Log out" button depending on `useAuth()`'s `user` state. There is deliberately no standalone "Medications" or "Upload" link - medication management, document upload, and analysis history all only exist within a patient's context (see Patients below), never as a global destination.
...
**`FileDropzone`** (`src/components/upload/FileDropzone.tsx`) has an optional `errorId` prop, included in the dropzone's own `aria-describedby`, so an unsupported-file-type error (`useDocumentQueue`'s `fileError`) is linked back to the control that caused it rather than floating as a `FormError` paragraph elsewhere on the page - a screen reader landing on the dropzone can tell why the last selection failed, matching how the medication CSV importer's dropzone already worked. `UploadPage` and `CreateAnalysisPage` pass a fixed id to both the prop and the paragraph's own `id`.
