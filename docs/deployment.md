...
| `OLLAMA_BASE_URL` | Backend **container runtime** | No, defaults to `http://host.docker.internal:11434` | Used only when `AI_PROVIDER=openbiollm` or `AI_PROVIDER=medgemma`, both served by a local Ollama daemon, not a hosted API - neither requires a credential. `host.docker.internal` is the address Docker Desktop (Mac/Windows) maps to the host machine from inside a container, since Ollama is expected to run on the host, not as a service in this compose file. Optional at the application level even when selected: the backend starts normally regardless of whether Ollama is reachable, and only an analysis request returns a `503` with a clear error naming the problem (see `docs/ai.md`). |
| `OPENBIOLLM_MODEL` | Backend **container runtime** | No, defaults to `openbiollm-llama3-instruct` | Which local Ollama model to call for OpenBioLLM - the locally-built, correctly-templated model, not the raw GGUF import (see `docs/ai.md`'s Manual Setup section). Only relevant when `AI_PROVIDER=openbiollm`. |
| `MEDGEMMA_MODEL` | Backend **container runtime** | No, defaults to `hf.co/bartowski/google_medgemma-4b-it-GGUF:Q4_K_M` | Which local Ollama model to call for MedGemma (see `docs/ai.md`). Only relevant when `AI_PROVIDER=medgemma`. |
...
**A note on `AI_PROVIDER=openbiollm`/`AI_PROVIDER=medgemma` in a real deployment:** unlike Gemini, these require a reachable Ollama server, which this compose file does not run as a service and does not provision - `host.docker.internal` (the `OLLAMA_BASE_URL` default above) only resolves from inside a container on Docker Desktop (Mac/Windows); it is not guaranteed on Linux Docker Engine (the typical EC2 setup below) without extra host-gateway configuration. Running either provider on a real EC2 instance means installing and running Ollama on that same instance (or another host reachable from it) yourself, and pointing `OLLAMA_BASE_URL` at it; this is genuinely more deployment work than an API key, and is not automated by anything in this repository today.
